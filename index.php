...
//------------ Assigned TPL File for Product and Badge Application --------------------------/
  ...
  $other_categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . (int)254 . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by c.sort_order, cd.categories_name");
  $homeProQuery = tep_db_query("SELECT p.products_id, p.products_image_med, pd.products_name, pc.categories_id FROM products as p LEFT JOIN products_description AS pd ON p.products_id = pd.products_id LEFT JOIN products_to_categories AS pc ON p.products_id = pc.products_id WHERE pd.products_name != '' AND p.products_parent_id = '0' AND p.products_id IN (86673,86676,86745,86758,86761,86764) 
