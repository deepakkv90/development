...
/*
  ...
  $Id: fss_functions.php,v 1.0.0.0 2006/10/21 10:20:48 datazen Exp $
...
*/
  ...
  function tep_fss_get_html($questions_id, $values = '') {
    global $_POST, $_SESSION, $_GET, $customer_id;
    $str = '';
    $questions = tep_db_fetch_array(tep_db_query("select questions_id, questions_type,     
questions_variable, questions_layout from " . TABLE_FSS_QUESTIONS . " where questions_id = '" . $questions_id . "'"));
//    print_r($values);die;
    if ($values == '') {
    ...
    if (tep_session_is_registered('customer_id')) {
      $customer_info = tep_fss_get_customer_info($customer_id);
    } else {
      $customer_info = array();
    }
    $last_value = tep_fss_get_last_value($questions_id, $name);
    switch ($questions['questions_type']) {
      case 'Input':
        ...
        $str = '<input name="' . $name . '" type="text"' . $values['property'] . ' /> ' . $values['append'] . $values['required'];
        break;
      case 'Hidden':
        if (!strstr($values['property'], 'value=')) {
          if ( isset($_GET[$name]) && tep_not_null($_GET[$name]) ) {
      ...
      case 'Check Box':
        ...
        foreach ($values['array'] as $key => $value) {
          $str .= '<td><input name="' . $name . '[]" type="checkbox" value="' . $value['value'] . '"' . ($last_value == $value['value'] ? ' checked="checked"' : '') . ' id="' . $name . '" /> &nbsp; </td>' . "\n" . '<td valign="top">' . $value['text'];
          if ($values['align'] == 'Vertical') {
            if ($key == (sizeof($values['array']) - 1)) {
//              $str .= '&nbsp;&nbsp;&nbsp;' . $values['append'] . $values['required'];
              $str .= $values['append'] . $values['required'];
              $str .= '</td></tr>' . "\n";
            } else {
              $str .= '</td></tr><tr>' . "\n";
        ...
        if ($values['align'] == 'Horizontal') {
         // $str .= '<td>&nbsp;&nbsp;&nbsp;' . $values['append'] . $values['required'] . '</td>';
          $str .= '</tr>' . "\n";
        }
        $str .= '</table>' . "\n";
        break;
      case 'Text Area':
        ...
        $str = '<textarea name="' . $name . '"' . $values['property'] . '>' . $values['internal'] . '</textarea> ' . $values['append'] . $values['required'];
        break;
      case 'File Upload':
        $str = '<input name="' . $name . '" type="file"' . $values['property'] . ' /> ' . $values['append'] . $values['required'];
        break;
      case 'Radio Button Group':        
        $str = '<table><tr>' . "\n";
        foreach ($values['array'] as $key => $value) {
          $str .= '<td style="padding-top:2px;"><input style="margin:0; padding:0;" name="' . $name . '" type="radio" value="' . $value['value'] . '"' . ($last_value == $value['value'] ? ' checked="checked"' : '') . ' /> &nbsp;</td>' . "\n" . '<td>' . $value['text'] . '&nbsp;';
          if ($values['align'] == 'Vertical') {
            if ($key == (sizeof($values['array']) - 1)) {
//              $str .= '&nbsp;&nbsp;&nbsp;' . $values['append'] . $values['required'];
              $str .= $values['append'] . $values['required'];
...
//        $str .= '</select>' . $values['append'] . $values['required'];
        $str .= '</select>' . $values['append'];
        break;
      case 'Drop Down Menu':
        $str = '<select name="' . $name . '" style="margin: 7px;"' . $values['property'] . '">';
        foreach ($values['array'] as $value) {
          $str .= '<option value="' . $value['value'] . '"' . ($last_value == $value['value'] ? ' selected' : '') . '>' . $value['text'] . '</option>' . "\n";
        }
//        $str .= '</select>' . $values['append'] . $values['required'];
    ...
    if (tep_not_null($values['required'])) {
      $required = true;
    } else {
      $required = false;
    }
    return array('str' => $str, 'required' => $required, 'param' => $values['param']);
  }
        ...
        switch ($value['fields_id']) {
          ...
          case '23':
            ...
            foreach ($name_value_array as $name_value) {
              ...
              $pair[] = array('text' => trim($pair_array[0]),
                              'value' => trim($pair_array[1]));
            }
            $str['array'] = $pair;
            break;
          case '15':
  ...
  function tep_fss_check_required($questions) {
    ...
    foreach ($questions as $question) {
      ...
      if ($question['html']['required'] && (!tep_not_null($_POST[$name]) || (!tep_fss_dropdown_menu_first_item_check($question_id, $_POST[$name])))) {
  ...
  function tep_fss_get_prefilled($prefilled_variable) {
    global $_GET, $customer_id;
    $array = explode('.', $prefilled_variable);
    $data = array();
    if (sizeof($array) > 1) {
      $table = $array[0];
      $field = $array[1];
      if ( tep_not_null($_GET['oID']) && tep_fss_has_field($table, array($field, 'orders_id')) ) {
        $data = tep_db_fetch_array(tep_db_query("select " . $field . " from " . $table . " where orders_id = '" . $_GET['oID'] . "'"));
      } elseif ( tep_not_null($_GET['products_id']) && tep_fss_has_field($table, array($field, 'products_id')) ) {
        $data = tep_db_fetch_array(tep_db_query("select " . $field . " from " . $table . " where products_id = '" . $_GET['products_id'] . "'"));
      } elseif ( tep_session_is_registered('customer_id') && tep_fss_has_field($table, array($field, 'customers_id')) ) {
        $data = tep_db_fetch_array(tep_db_query("select " . $field . " from " . $table . " where customers_id = '" . $customer_id . "'"));
      }      
    }    
    return $data[$field];
  }
  ...
  function tep_fss_get_last_value($questions_id, $name) {
    global $customer_id;
    if (isset($_POST[$name]) && tep_not_null($_POST[$name])) {
      $ret = $_POST[$name];
    } elseif (tep_session_is_registered('customer_id')) {
      $data = tep_db_fetch_array(tep_db_query("select ffpc.forms_fields_value from " . TABLE_FSS_FORMS_POSTS_CONTENT . " ffpc, " . TABLE_FSS_FORMS_POSTS . " ffp where ffp.forms_posts_id = ffpc.forms_posts_id and ffpc.questions_id = '" . $questions_id . "' and ffp.customers_id = '" . $customer_id . "' order by ffp.posts_date desc"));
