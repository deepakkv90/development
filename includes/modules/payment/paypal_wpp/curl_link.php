<?php
/*
  $Id: curl_link.php,v 1.0.0.0 2007/11/13 13:41:11 datazen Exp $
...
class curl_link {
  ...
  function curl_link($url, $cookie_name = '/tmp/paypal_wpp_cookie.tmp') {
    if (!function_exists('curl_init')) {
      trigger_error('curl_link::error, Sorry but it appears that CURL is not loaded, Please install it to continue.');
      return false;
    }
    if (empty($url)) {
//      trigger_error('curl_link::error, The link exchange website location is required to continue, Please edit your script.');
      return false;
    }
    $this->url = $url;
...
//    curl_setopt($this->curl, CURLOPT_TIMEOUT, 5);
  ...
  function write_log($message, $log_file = 'debug/paypal_wpp_debug.txt') {
    $fp = @fopen($log_file, 'a');
