  ...
  class packing {
    ...
    global $shipping_weight, $shipping_num_boxes, $total_weight, $boxcount, $cart, $order;
        ...
        foreach ($productsArray as $key => $value) {
          $dimension_query = tep_db_query("SELECT * FROM " . TABLE_UPSXML_PRODUCTS_DIMENSION . " WHERE products_id = '" . (int)$value['id'] . "'");
          if (tep_db_num_rows($dimension_query) > 0) {
            $dimension = tep_db_fetch_array($dimension_query);
            $productsArray[$key]['length'] = $dimension['products_length'];
            $productsArray[$key]['height'] = $dimension['products_height'];
            $productsArray[$key]['width'] = $dimension['products_width'];
            $productsArray[$key]['ready_to_ship'] = $dimension['products_ready_to_ship'];
          } else {
            $productsArray[$key]['length'] = '';
            $productsArray[$key]['height'] = '';
            $productsArray[$key]['width'] = '';
            $productsArray[$key]['ready_to_ship'] = '0';
          }
        }
    ...
    function getPackagesByVol() {
        $packages = array();
        $packages_query = tep_db_query("select *, (package_length * package_width * package_height) as volume from " . TABLE_UPSXML_PACKAGING . " order by volume");
        $counter = 0;
...
//            print("ERROR: No boxes to ship unpackaged product<br>\n");
        ...
        while (count($productsRemaining) && count($packedBoxesArray) < 200) {
...
// ******************************
if(!function_exists('ready_to_shipCmp')){
