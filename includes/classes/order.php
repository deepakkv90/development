...
where op.orders_id = '" . (int)$order_id . "' and op.products_id=p.products_id order by op.orders_products_id");
	...
	global $billto, $cart, $languages_id, $currency, $currencies, $shipping, $statea, $country_title_s, $format_id_s, $customer_id;
      ...
      if(NBI_SYNC==true) {
		  ...
		  $this->info['source'] = NBI_SOURCE_CODE;
	  ...
	  $pro_qry = tep_db_query("select * from products where products_id = ".$products[$i]['id']);

      $pro_rst = tep_db_fetch_array($pro_qry);

	  $pro_cat_qry = tep_db_query("select categories_id from products_to_categories where products_id = ".$products[$i]['id']);

      $pro_cat_rst = tep_db_fetch_array($pro_cat_qry);
		...
		$this->products[$index] = array('qty' => $products[$i]['quantity'],
									...
									'categories_id' => $pro_cat_rst["categories_id"],
									
									'products_quantity' => $pro_rst["products_quantity"],
									
									'products_image' => $pro_rst["products_image"],
									
									'products_image_med' => $pro_rst["products_image_med"],
									
									'products_image_lrg' => $pro_rst["products_image_lrg"],
									
									'products_price' => $pro_rst["products_price"],
									
									'products_date_added' => $pro_rst["products_date_added"],
									
									'products_last_modified' => $pro_rst["products_last_modified"],
									
									'products_date_available' => $pro_rst["products_date_available"],
									
									'products_status' => $pro_rst["products_status"],
									
									'manufacturers_id' => $pro_rst["manufacturers_id"],
									
									'products_tax_class_id' => $pro_rst["products_tax_class_id"],
									
									'products_ordered' => $pro_rst["products_ordered"],
									
									'products_parent_id' => $pro_rst["products_parent_id"],
									
									'user_id' => $pro_rst["user_id"],
									
									'badge_namefile' => $pro_rst["badge_namefile"],
									
									'badge_logo' => $pro_rst["badge_logo"],
									
									'badge_data' => $pro_rst["badge_data"],
									
									'default_product_id' => $pro_rst["default_product_id"],
									
									'max_lines_count' => $pro_rst["max_lines_count"],
									
									'max_images_count' => $pro_rst["max_images_count"],
									
									'default_texts' => $pro_rst["default_texts"],
									
									'products_group_access' => $pro_rst["products_group_access"],
									
									'products_nav_access' => $pro_rst["products_nav_access"],
									
									'products_text' => $pro_rst["products_text"],
									
									'badge_comment' => $pro_rst["badge_comment"],
									
									'labour_cost' => $pro_rst["labour_cost"],
									
									'material_cost' => $pro_rst["material_cost"],
									
									'overhead_cost' => $pro_rst["overhead_cost"],
									
									'overall_quantity' => $pro_rst["overall_quantity"],
									
									'progressed_quantity' => $pro_rst["progressed_quantity"],

									'id' => $products[$i]['id']);

        $products_id_tmp = tep_get_prid($products[$i]['id']);
		
		/* Get products badge options */
		
		$pbo_qry = tep_db_query("Select * from products_badge_options WHERE products_id = '".$products[$i]['id']."'");
		
		$op=0;
		
		while($pbo_arr = tep_db_fetch_array($pbo_qry)) {
			
			$this->products[$i]['options'][$op] = array('products_id' => $products[$i]['id'],

									'options_id' => $pbo_arr['options_id'],
									
									'options_type' => $pbo_arr['options_type'],
									
									'options_values_id' => $pbo_arr['options_values_id'],
									
									'options_text' => $pbo_arr['options_text'],

									'options_desc' => $pbo_arr['options_desc']);
