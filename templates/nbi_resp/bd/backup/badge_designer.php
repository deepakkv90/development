        ...
        for ($img_arr = 0; $img_arr < $max_count; $img_arr++) {
			...
			foreach ($badge_data->texts as $text) {
			  ...
			  if ($colors = badge_str2rgb($text->color)) {
				$color = imagecolorallocate($new_image, $colors[1], $colors[2], $colors[3]);            
				$box = @imagettfbbox(px2pt($text_size), 0, $font_name, properText($text->text));			
				...
				$x = $x*$res_x;
				$y = $y*$res_y;            
				imagettftext($new_image, px2pt($text_size), $angle, $x, $y, $color, $font_name, properText($text->text));						
			  }
			  $cust = $cust + 1;  
			...
			if ($temp_pict = post('temp_pict', get('temp_pict'))) {
			  ...
			  if(isset($textString)){
					$TextContent  = properText($textString);
			  }else{
					$TextContent  = "";
			  }
