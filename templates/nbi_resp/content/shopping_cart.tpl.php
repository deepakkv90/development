...
<?php
	...
	<tr class="woocommerce-cart-form__cart-item cart_item">
		...
		<td class="quantity product-quantity"><?php echo tep_draw_input_field('cart_quantity['.$i.']', $products[$i]['quantity'], 'class="w30" size="4" '.$min_qty_attr) . $min_qty_label . tep_draw_hidden_field('products_id[]', $products[$i]['id_string']); ?>
			&nbsp;
			<img type="image" title="Update" alt="Update" src="<?php echo DIR_WS_TEMPLATES . TEMPLATE_NAME; ?>/image/update.png" onclick="$('#frm_shopping_cart').submit();">
			&nbsp;
			<?php echo tep_draw_checkbox_field('cart_delete[]', $products[$i]['id_string'], false, ' style="display:none;" id="delete_'.$i.'"'). '<img type="image" title="Delete" src="'.DIR_WS_TEMPLATES.TEMPLATE_NAME.'/image/remove.png" onclick="'. "$('#delete_".$i."').attr('checked', 'checked'); $('#frm_shopping_cart').submit();" . '">';?>
		</td>
...
if (RETURN_CART == L){
   $back = sizeof($navigation->path)-2;
    ...
    $nav_link = '<a class="button ss" href="' . tep_href_link($navigation->path[$back]['page'], tep_array_to_string($navigation->path[$back]['get'], array('action')), $navigation->path[$back]['mode']) . '">Continue Shopping</a>';
        }
  ...
  if (!eregi('wishlist', $_SERVER['HTTP_REFERER'])) {
    ...
    if ($products == '') {
      ...
      $nav_link = '<a class="button aa" href="' . tep_href_link(FILENAME_DEFAULT) . '">Continue Shopping</a>'  ;
    }
  }else{
    $nav_link = '<a class="button dd" href="' . tep_href_link(FILENAME_DEFAULT) . '">Continue Shopping</a>'  ;
  }
