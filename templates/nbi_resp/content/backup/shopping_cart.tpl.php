	...
	<table>
	  <thead>
		<tr>
		  ...
		  <td class="name"><?php echo TABLE_HEADING_PRODUCTS; ?></td>
		  <td class="quantity"><?php echo TABLE_HEADING_QUANTITY; ?></td>
		  <td class="price"><?php echo TABLE_HEADING_UNIT_PRICE; ?></td>
		  <td class="total"><?php echo TABLE_HEADING_SUB_TOTAL; ?></td>
		</tr>
	  </thead>
	  <tbody>
...
<?php
	...
	?>
	<tr>
		<td class="image"> <?php echo $products_img; ?></td>
		<td class="name"> <?php echo $products_name; ?></td>
		<td class="quantity"><?php echo tep_draw_input_field('cart_quantity['.$i.']', $products[$i]['quantity'], 'class="w30" size="4" '.$min_qty_attr) . $min_qty_label . tep_draw_hidden_field('products_id[]', $products[$i]['id_string']); ?>
			&nbsp;
			<input type="image" title="Update" alt="Update" src="<?php echo DIR_WS_TEMPLATES . TEMPLATE_NAME; ?>/image/update.png">
			&nbsp;
			<?php echo tep_draw_checkbox_field('cart_delete[]', $products[$i]['id_string'], false, ' style="display:none;" id="delete_'.$i.'"'). '<input type="image" title="Delete" src="'.DIR_WS_TEMPLATES.TEMPLATE_NAME.'/image/remove.png" onclick="'. "$('#delete_".$i."').attr('checked', 'checked');" . '">';?>
		</td>
		<td class="price"><?php echo $currencies->display_price($products[$i]['final_price']); ?></td>
		<td class="total"><?php echo $currencies->display_price($products[$i]['final_price'], '',$products[$i]['quantity']); ?></td>
	</tr>
	<?php 
	}
	?>
...
?>
      ...
      if (trim(strip_tags($returned_rci)) != NULL) {
        ...
        echo '  <table id="total">' . "\n";
        echo '    <tr>' . "\n";
        echo '      <td class="right"><b>' . SUB_TITLE_SUB_TOTAL . '</b></td>' . "\n";
        echo '      <td class="right"><b>' . $currencies->format($cart->show_shopping_cart_total()) . '</b></td>' . "\n";
        echo '    </tr>' . "\n";
		
		echo '    <tr>' . "\n";
        echo '      <td class="right"><b>'.SUB_TITLE_GRAND_TOTAL_EXCL_TAX.'</b></td>' . "\n";
        echo '      <td class="right"><b>' . $currencies->format($cart->show_shopping_cart_total()) . '</b></td>' . "\n";
        echo '    </tr>' . "\n";   
		
		echo '    <tr>' . "\n";
        echo '      <td class="right"><b>'.SUB_TITLE_TAX.'</b></td>' . "\n";
        echo '      <td class="right"><b>' . $currencies->format($gst_tax) . '</b></td>' . "\n";
        echo '    </tr>' . "\n";   
        echo $returned_rci;
        echo '    <tr>' . "\n";
        echo '      <td class="right"><b>' . SUB_TITLE_GRAND_TOTAL_INCL_TAX . '</b></td>' . "\n";
        echo '      <td class="right"><b>' . $currencies->format($cart->show_shopping_cart_total() + $offset_amount + $gst_tax) . '</b></td>' . "\n";
        echo '    </tr>' . "\n";
        echo '  </table>' . "\n";
		...
		echo '    <tr>' . "\n";
        echo '      <td class="right"><b>'.SUB_TITLE_GRAND_TOTAL_INCL_TAX.'</b></td>' . "\n";
        echo '      <td class="right"><b>' . $currencies->format($cart->show_shopping_cart_total() + $gst_tax) . '</b></td>' . "\n";
        echo '    </tr></tbody>' . "\n";
		echo '  </table>' . "\n";
