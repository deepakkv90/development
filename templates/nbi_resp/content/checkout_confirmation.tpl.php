	...
	<table>
	  ...
	  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
			...
			if ($dprid) {				
				...
				$badge = new Badge($product_arr['badge_data']);
				$products_name .= '<div class="badge-desc">'.$badge->short_description() . tep_get_badge_products_options($order->products[$i]['id']) .'</div>';
			} 
