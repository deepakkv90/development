      ...
      case 'save':
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
          ...
          $sql_data_array = array('orders_status_name' => tep_db_prepare_input($orders_status_name_array[$language_id]),
								'sort_order' => tep_db_prepare_input($sort_order_array[$language_id]));
...
<?php
  $orders_status_query_raw = "select orders_status_id, orders_status_name, sort_order from " . TABLE_ORDERS_STATUS . " where language_id = '" . (int)$languages_id . "' order by sort_order";
...
?>
                <td class="dataTableContent" align="center"><?php echo $orders_status['sort_order']; ?></td>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $orders_status_inputs_string .= '<br>' . tep_image(HTTP_SERVER . DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('orders_status_name[' . $languages[$i]['id'] . ']') . '<br>Sort Order' . tep_draw_input_field('sort_order[' . $languages[$i]['id'] . ']');
      }
    ...
    case 'edit':
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $orders_status_inputs_string .= '<br>' . tep_image(HTTP_SERVER . DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('orders_status_name[' . $languages[$i]['id'] . ']', tep_get_orders_status_name($oInfo->orders_status_id, $languages[$i]['id'])). '<br>Sort Order' . tep_draw_input_field('sort_order[' . $languages[$i]['id'] . ']',tep_get_orders_status_sort_order($oInfo->orders_status_id, $languages[$i]['id']));
      }
