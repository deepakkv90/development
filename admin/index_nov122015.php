...
//TEmplate Check code
  ...
  if(empty($_SESSION["os"])) {
  	$_SESSION["os"] = array(1, 2, 4, 6, 100001, 100002, 100003, 100004, 100008, 100009);	
  }   
  ...
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
	...
	//add selected or default orders status to chart
		foreach($os_chart as $key=>$value) {			
			if($value==$status_id) {
				$orders_status_graph .= '{linkto:"'.HTTP_SERVER . DIR_WS_HTTP_ADMIN . FILENAME_ORDERS .'?selected_box=customers&status=' . $status_id .'&osCAdminID='.$sesid.'", country:"'.$status_name.'", visits:'.$orders_pending['count'].', color:"'.$colors_array[$s].'"},';
				$s++;
...
//get artwork status
if(isset($_GET['action']) && $_GET['action']=="art_chart") {  		
	
	$art_from_1 = $_POST['artworks_from'];	
	$art_to_1 = $_POST['artworks_to'];
	
	$art_from = "";
	if(isset($_POST['artworks_from']) && !empty($_POST['artworks_from'])){       
	  $art_from = date("Y-m-d H:i:s", strtotime(tep_db_input($_POST['artworks_from'])));
    }	
	
	$art_to = "";
	if(isset($_POST['artworks_to']) && !empty($_POST['artworks_to'])){  
	  $art_to = date("Y-m-d H:i:s", strtotime("+1 day",strtotime(tep_db_input($_POST['artworks_to']))));
    }
		
	$artwork_filter = "";
	if($art_to!="" && $art_from!="") {
		$artwork_filter = " AND (a.date_created >= '".$art_from."' AND a.date_created < '".$art_to."') ";
	}
	
} 

$artwork_pending_query = tep_db_query("SELECT count(a.customers_id) as count FROM artwork a LEFT JOIN customers c ON a.customers_id = c.customers_id WHERE a.artwork_status  = 'pending' AND c.customers_id IS NOT NULL ".$artwork_filter);
$artwork_revision_query = tep_db_query("SELECT count(a.customers_id) as count FROM artwork a LEFT JOIN customers c ON a.customers_id = c.customers_id WHERE a.artwork_status  = 'revision' AND c.customers_id IS NOT NULL  ".$artwork_filter);
$artwork_approved_query = tep_db_query("SELECT count(a.customers_id) as count FROM artwork a LEFT JOIN customers c ON a.customers_id = c.customers_id WHERE a.artwork_status  = 'approved' AND c.customers_id IS NOT NULL  ".$artwork_filter);
	
$artwork_pending = tep_db_fetch_array($artwork_pending_query);
$artwork_revision = tep_db_fetch_array($artwork_revision_query);
$artwork_approved = tep_db_fetch_array($artwork_approved_query);
			...
			<table align="center" width="100%" border="0">
				<tr>
					...
					<td width="25%" valign="top">										
						<fieldset>
							<legend><?php echo "Artwork Status";?> </legend>
							
							<?php echo tep_draw_form('frmArtwork', FILENAME_DEFAULT, tep_get_all_get_params(array('action')) . 'action=art_chart', 'post', '', 'SSL'); ?>
							<div style="background:#FFF url(images/pallate-40.png) 0px 0px no-repeat; position:absolute; z-index:10; width:40px;height:40px;">&nbsp;</div>
							<br><br>
							<div style=" z-index:5;">						
							...
							<table align="center" width="100%" border="0">
								 <tr>								
									<td colspan="4" class="main"><b>Select duration </b></td>
								 </tr>
								 <tr>
									<td class="main" width="10%"><b>From: </b></td>
									<td class="main"><input type="text" id="datepicker_from" name="artworks_from" value="<?php echo (isset($art_from_1))? $art_from_1 : ""; ?>" style="width:70px;"></td>
									<td class="main" width="5%"><b>To: </b></td>
									<td class="main"><input type="text" id="datepicker_to" name="artworks_to" value="<?php echo (isset($art_to_1))? $art_to_1 : ""; ?>" style="width:70px;"></td>
								 </tr>							 
								  <tr>								
									<td colspan="4" align="center" class="main">
										<?php echo tep_image_submit('button_submit.gif', "Submit");  ?>
										&nbsp;
										<?php echo '<a href="' . tep_href_link(FILENAME_DEFAULT, '', 'SSL') . '">' . tep_image_button('button_reset.gif', "Reset") . '</a>'; ?>
									</td>
								 </tr>
							</table>
							</div>
