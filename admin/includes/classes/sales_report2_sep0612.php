...
*/
  ...
  class sales_report {
    ...
    function sales_report($mode, $startDate = 0, $endDate = 0, $sort = 0, $statusFilter = 0, $filter = 0, $details) {
	  ...
	  $this->queryOrderProCostCnt = "SELECT (opc.labour_cost*opc.products_quantity) as pcl_cost, (opc.overhead_cost*opc.products_quantity) as pco_cost, (opc.material_cost*opc.products_quantity) as pcm_cost, o.date_purchased, o.customers_id, a.entry_zone_id FROM orders_products_costs opc, orders o, address_book a WHERE opc.orders_id=o.orders_id AND o.customers_id = a.customers_id ";
	  
	  //$this->queryItemCnt_2 = "SELECT sum(op.products_quantity) as items_cnt FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op WHERE o.orders_id = op.orders_id ";
	  
	  $this->queryItemCnt_2 = "SELECT p.products_id, p.products_model, op.products_quantity, (op.final_price*op.products_quantity) as psum, o.date_purchased FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_PRODUCTS . " p WHERE o.orders_id = op.orders_id AND op.products_id = p.products_id ";
      ...
      // query for shipping
      $this->queryShipping = "SELECT ot.value as shipping, o.date_purchased, o.customers_id, a.entry_zone_id FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot, address_book a WHERE ot.orders_id = o.orders_id AND  ot.class = 'ot_shipping' AND o.customers_id = a.customers_id ";
	  
	   // query for GST total
      $this->queryGstTotal = "SELECT sum(ot.value) as gst_total FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot WHERE ot.orders_id = o.orders_id AND  ot.class = 'ot_gst_total'";
	  
	 $this->queryDiscountTotal = "SELECT sum(ot.value) as discount_total FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot WHERE ot.orders_id = o.orders_id AND  (ot.class = 'ot_customer_discount' OR ot.class='ot_gv')";
    ...
    function getNext() {
	 ...
	 $dateFilters = " (o.date_purchased >= '" .tep_db_input(date("Y-m-d\TH:i:s", $sd)). "' AND o.date_purchased < '" .tep_db_input(date("Y-m-d\TH:i:s", $ed)). "') ";
	  ...
	  if($this->details==0) {		  
		  ...
		  $rqOrders = tep_db_query($this->queryOrderCnt . " WHERE " . $dateFilters . $filterInvoiceString . $filterString);
			  $order = tep_db_fetch_array($rqOrders);	
			  
			  $rqOrdersItems = tep_db_query($this->queryItemCnt_2 . " AND " . $dateFilters . $filterInvoiceString . $filterString);	
			  ...
			  $rqShipping = tep_db_query($this->queryShipping . " AND " . $dateFilters . $filterInvoiceString . $filterString);
			  ...
			  $rqGstTotal = tep_db_query($this->queryGstTotal . " AND  " . $dateFilters . $filterInvoiceString . $filterString);		  
			  while($gst_total = tep_db_fetch_array($rqGstTotal)) {
					$gst_cost += $gst_total["gst_total"];
			  }		  
			  $rqDiscountTotal = tep_db_query($this->queryDiscountTotal . " AND " . $dateFilters . $filterInvoiceString . $filterString);
			  while($discount_total = tep_db_fetch_array($rqDiscountTotal)) {
					$discount_cost += $discount_total["discount_total"];
			  }			
			  
			  //echo $this->querySubtotal . " AND " . $dateFilters . $filterInvoiceString . $filterString . "GROUP BY o.orders_id";
			  //echo "<br><br>";  
			  $rqSubTotal = tep_db_query($this->querySubtotal . " AND " . $dateFilters . $filterInvoiceString . $filterString . "GROUP BY o.orders_id");
			  ...
			  $rqOrdProCost = tep_db_query($this->queryOrderProCostCnt . " AND " . $dateFilters . $filterInvoiceString . $filterString);
	  ...
	  if($this->details==1 || $this->details==2 || $this->details==3) {
			$rqItems = tep_db_query($this->queryItemCnt ." AND " . $dateFilters . $filterInvoiceString . $filterString." group by orders_products_id " . $this->sortString);
			$this->rqOrdProCostInfo = tep_db_query($this->queryProCost_2 . " AND " . $dateFilters . $filterInvoiceString . $filterString);			  
	  	    $this->rqOrdProCostDetail = tep_db_query($this->queryProCost . " ( SELECT o.orders_id from orders o where " . $dateFilters . $filterInvoiceString . $filterString.")"); 
	  }
	  ...
	  if($this->details==1 || $this->details==2 || $this->details==3) {
		  ...
		  while ($resp[$cnt] = tep_db_fetch_array($rqItems)) {
			...
			//$price = $resp[$cnt]['psum'] / $resp[$cnt]['pquant']; // modified Aug 18, 2010
			$price = $resp[$cnt]['final_price']; 
			
			//below variables assigned below - modified Aug 18, 2010
			/*
			...
			*/
			...
			$rqAttr = tep_db_query($this->queryAttr . " AND " . $dateFilters . $filterInvoiceString . " AND op.products_id = " . $resp[$cnt]['pid'] . $filterString . " group by products_options_values order by orders_products_id");
			$i = 0;
