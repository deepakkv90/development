    ...
    /*  $totals_query = tep_db_query("select title, text from " . TABLE_RETUR_TOTAL . " where returns_id = '" . tep_db_input($returns_id) . "' order by sort_order");
     while ($totals = tep_db_fetch_array($totals_query)) {
      ...
      $this->info = array('currency' => $order['currency'],
                          ...
                          'date_finished' => (isset($order['date_finished']) ? $order['date_finished'] : ''),
                          //
...
// BEGIN - Product Serial Numbers
      $orders_products_query = tep_db_query("select returns_id, returns_products_id, products_id, products_name, products_model, products_price, products_tax, products_quantity, final_price, products_serial_number, products_discount_made  from " . TABLE_RETURNS_PRODUCTS_DATA . " where returns_id ='" . tep_db_input($returns_id) . "'");
// END - Product Serial Numbers
      ...
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
        ...
        $attributes_query = tep_db_query("SELECT *
                                          FROM " . TABLE_RETURNS_PRODUCTS_ATTRIBUTES_DATA . "
                                          WHERE returns_id = " . (int)$orders_products['returns_id'] . "
                                          and returns_products_id = '" . (int)$orders_products['returns_products_id'] . "'
                                          ORDER BY products_options, products_options_values");
        if (tep_db_num_rows($attributes_query)) {
          while ($attributes = tep_db_fetch_array($attributes_query)) {
            $attributes_array[] = array('option_id' => $attributes['products_options_id'],
                                'value_id' => $attributes['products_options_values_id'],
                                'value' => $attributes['products_options_values'],
                                'option' => $attributes['products_options'],
                                'option_name' => $attributes['products_options'],
                                'prefix' => $attributes['price_prefix'],
                                'price' => $attributes['options_values_price']);
          }
          $this->products['attributes'] = $attributes_array;
        }
      }
    }
  }
