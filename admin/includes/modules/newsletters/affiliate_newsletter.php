  ...
  class affiliate_newsletter {
    ...
    function confirm() {
      ...
      $confirm_string = '<table border="0" cellspacing="0" cellpadding="2">' . "\n" .
                        ...
                        '  </tr>' . "\n" .
                        '  <tr>' . "\n" .
                        '    <td align="right"><a href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>&nbsp;&nbsp;'. ' <a href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID'] . '&action=confirm_send') . '">' . tep_image_button('button_send.gif', IMAGE_SEND) . '</a></td>' . "\n" .
                        '  </tr>' . "\n" .
                        '</table>';
    ...
    function send($affiliate_newsletter_id) {  
      ...
      while ($mail = tep_db_fetch_array($mail_query)) {
        $mimemessage->send($mail['affiliate_firstname'] . ' ' . $mail['affiliate_lastname'], $mail['affiliate_email_address'], STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $this->title);
      }
