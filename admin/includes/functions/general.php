...
// Text Mode
  ...
  function tep_get_orders_status_sort_order($orders_status_id, $language_id = '') {

    global $languages_id;

    if (!$language_id) $language_id = $languages_id;

    $orders_status_query = tep_db_query("select sort_order from " . TABLE_ORDERS_STATUS . " where orders_status_id = '" . (int)$orders_status_id . "' and language_id = '" . (int)$language_id . "'");

    $orders_status = tep_db_fetch_array($orders_status_query);

    return $orders_status['sort_order'];
...
// TABLES: products, products_to_categories, categories
  ...
  function tep_products_in_category_count($categories_id, $include_deactivated = false) {
    ...
    if ($include_deactivated) {
      ...
      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$categories_id . "' AND (p.default_product_id is NULL OR p.default_product_id=0)");

    } else {

      $products_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = p2c.products_id and p.products_status = '1' and p2c.categories_id = '" . (int)$categories_id . "' AND (p.default_product_id is NULL OR p.default_product_id=0)");
...
$kernel = '';
  ...
  function tep_remove_order($order_id, $restock) {
	...
	//Start update customers orders count - Sep 12 2011
	$customer_query = tep_db_query("SELECT customers_id from ".TABLE_ORDERS." WHERE orders_id = '" . (int)$order_id . "'");
	$customer = tep_db_fetch_array($customer_query);	
	$customer_orders = tep_db_query("SELECT customers_orders_count from customers WHERE customers_id = '" . (int)$customer['customers_id'] . "'");
	$order_count = tep_db_fetch_array($customer_orders);
	if($order_count['customers_orders_count']>0) {
		$current_count = (int)($order_count['customers_orders_count']-1);
		tep_db_query("update customers set customers_orders_count='".$current_count."' WHERE customers_id='".(int)$customer['customers_id']."'");
	}
	//End update customers orders count - Sep 12 2011
	...
	tep_db_query("delete from orders_products_costs where orders_id = '" . (int)$order_id . "'");
...
// Output the tax percentage with optional padded decimals
  ...
  function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address) {
   ...
   function tep_mail_with_attach($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address, $attach = null) {
    ...
    if($attach){
      ...
      if(is_array($attach)){
        ...
        foreach ($attach as $att){
          ...
          if(substr($att['file'], 0 , -3) == 'png'){                         

            $message->add_attachment($att['file'], $att['name'], 'image/png');           

          } else {                         ;

              $message->add_attachment($att['file'], $att['name']);              
...
function func_category_group_access($group_access,$current_category_id) {
  ...
  <table width="100%" border = "0" cellspacing="0" cellpadding="0" align="center" >
    ...
    <td>  <fieldset style="display:none;"><legend></legend></fieldset>  <fieldset><legend><?php echo TEXT_PUSH_SAVE_OPTION;?></legend>
...
//functions used in artwork and design start

function tep_get_all_admin() {
	$sqlSelAdmin = tep_db_query("SELECT * FROM ".TABLE_ADMIN);
	$admin_arr = array();
	while($admin = tep_db_fetch_array($sqlSelAdmin)) {
		$admin_arr[] = $admin; 
	}
	return $admin_arr;
}

function tep_get_order_products($op_id) {
	$sqlSelOrdPro = tep_db_query("SELECT * FROM ".TABLE_ORDERS_PRODUCTS." WHERE orders_products_id='".$op_id."'");	
	$op = tep_db_fetch_array($sqlSelOrdPro);	
	return $op;
}

function tep_get_artwork_option($artwork_id) {
	$sel_option= tep_db_query("SELECT * FROM artwork_option WHERE artwork_id='".$artwork_id."' ORDER BY artwork_option_id ASC");
	while($rst_arr_opt = tep_db_fetch_array($sel_option)) {
		$opt_arr[] = $rst_arr_opt;
	}
	return $opt_arr;
}

//get option infor by option id
...
function tep_get_revision_count($option_id) {
	$sel_revision= tep_db_query("SELECT COUNT(*) as rev_count FROM artwork_option WHERE revision_id='".$option_id."'");
	$rev_count = tep_db_fetch_array($sel_revision);	
	return $rev_count['rev_count'];
}

//artwork revision count
function tep_get_artwork_revision_count($art_id) {
	
	$sel_revision= tep_db_query("SELECT COUNT(*) as rev_count FROM artwork_feedback WHERE status='revision' and artwork_id='".$art_id."'");
	$rev_count = tep_db_fetch_array($sel_revision);		
	return $rev_count['rev_count'];
}

function tep_get_artwork_option_count($artwork_id) {
	$sel_option= tep_db_query("SELECT COUNT(*) as opt_count FROM artwork_option WHERE artwork_id='".$artwork_id."'");
	$opt_count = tep_db_fetch_array($sel_option);		
	return $opt_count['opt_count'];
}

function tep_get_admin_details($admin_id) {
	$sqlSelOrdPro = tep_db_query("SELECT * FROM ".TABLE_ADMIN." WHERE admin_id='".$admin_id."'");	
	$op = tep_db_fetch_array($sqlSelOrdPro);	
	return $op;
}

function tep_date_aus_format($date,$type) {
	...
	if($type=="short") {
		...
		return date("d/m/Y H:i:g", strtotime($date));
	}
}
//functions used in artwork and design end


// Products costs functions 
//get products cost value
function tep_get_products_costs($id) {
	$sel_products_costs = tep_db_query("SELECT labour_cost, overhead_cost, material_cost FROM products WHERE products_id='".$id."'");
	$rst_arr_opt = tep_db_fetch_array($sel_products_costs);	
	return $rst_arr_opt;
}

function tep_get_orders_products_costs($order_id) {
	$sel_products_costs = tep_db_query("SELECT * FROM orders_products_costs WHERE orders_id='".$order_id."'");
	if(tep_db_num_rows($sel_products_costs)==0) {
		return false;
	}
	
	$total_costs = 0;
	
	while($rst_arr_opt = tep_db_fetch_array($sel_products_costs)) {
		$material_cost = ($rst_arr_opt['material_cost'] * $rst_arr_opt['products_quantity']);
		$labour_cost = ($rst_arr_opt['labour_cost'] * $rst_arr_opt['products_quantity']);
		$overhead_cost = ($rst_arr_opt['overhead_cost'] * $rst_arr_opt['products_quantity']);
		$total_costs += ($material_cost + $labour_cost + $overhead_cost);
						
		$opt_arr[] = $rst_arr_opt;
	}
	
	$opt_arr['total_costs'] = $total_costs;
	
	return $opt_arr;
}

function tep_get_orders_products_name($orders_products_id, $orders_id) {
	$product_query = tep_db_query("select products_name from orders_products where orders_products_id = '" .(int)$orders_products_id . "' and orders_id = '" . (int)$orders_id ."'");
    $product = tep_db_fetch_array($product_query);        
	return $product['products_name'];
}


function tep_get_product_category($products_id) {
    ...
    if ($cat_count_data['count'] == 1) {
      ...
      $catID = $cat_id_data['categories_id'];
      $cat_id_desc = tep_db_query("select c.parent_id, cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and c.categories_id = '" . $catID . "'");
      $cat_id_data = tep_db_fetch_array($cat_id_desc);     	  
	  $cat_info['name'] = $cat_id_data['categories_name'];	  
	  $cat_info['id'] = $catID;	  
	  $cat_info['parent_id'] = $cat_id_data['parent_id'];	  
	  $parent_qry = tep_db_query("SELECT categories_name FROM ". TABLE_CATEGORIES_DESCRIPTION . " WHERE categories_id = '" . $cat_info['parent_id'] . "'");	  
	  $parent_data = tep_db_fetch_array($parent_qry); 
	  $cat_info['parent_name'] = $parent_data['categories_name'];    
...
// TABLES: manufacturers
  function tep_get_manufacturer_name($manufacturer_id) {  	
   ...
   function tep_get_product_manufacturer($products_id) {  	
    $manufacturer_query = tep_db_query("select manufacturers_id from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
    $manufacturer = tep_db_fetch_array($manufacturer_query);		
	return $manufacturer['manufacturers_id'];
  }

  //get product model  
  function tep_get_product_model($products_id) {  	
    $models_query = tep_db_query("select products_model from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
    $models = tep_db_fetch_array($models_query);		
	return $models['products_model'];
  }	
  
/////  

// Products costs functions eof 

  //Artwork resolution
//get artwork option high resolution image by artwork option id
function tep_get_resolution($option_id) {
	$sel_option= tep_db_query("SELECT * FROM artwork_option_resolution WHERE artwork_option_id='".$option_id."'");
	$option = tep_db_fetch_array($sel_option);	
	return $option;
}

//get artwork feedback by option
function tep_get_option_feedback($option_id) {
	$sel_option= tep_db_query("SELECT * FROM artwork_feedback WHERE artwork_option_id='".$option_id."'");
	$option = tep_db_fetch_array($sel_option);	
	return $option;
}
...
function get_zone_by_post_code($postcode) {
	
	if(($postcode <= 4999 && $postcode >= 4000) || ($postcode <= 9999 && $postcode >= 9000)) {
		$zone_id = "182";
	}
	else if(($postcode <= 1999 && $postcode >=1000) || ($postcode <= 2599 && $postcode >= 2000) || ($postcode <= 2898 && $postcode >= 2619) || ($postcode <= 2999 && $postcode >= 2921)) {		
		$zone_id = "184";
	}
	else if(($postcode <= 3999 && $postcode >= 3000) || ($postcode <= 8999 && $postcode >= 8000)) {	
		$zone_id = "185";
	}
	else if(($postcode <= 5799 && $postcode >= 5000) || ($postcode <= 5999 && $postcode >= 5800)) {		
		$zone_id = "186";
	}
	else if(($postcode <= 6797 && $postcode >= 6000) || ($postcode <= 6999 && $postcode >= 6800)) {		
		$zone_id = "187";
	}
	else if(($postcode <= 299 && $postcode >= 200) || ($postcode <= 2618 && $postcode >= 2600) || ($postcode <= 2920 && $postcode >= 2900)) {		
		$zone_id = "188";
	}
	else if(($postcode <= 899 && $postcode >= 800) || ($postcode <= 999 && $postcode >= 900)) {		
		$zone_id = "189";
	}
	else if(($postcode <= 7799 && $postcode >= 7000) || ($postcode <= 7999 && $postcode >= 7800)) {		
		$zone_id = "190";
	}
	else {		
		$zone_id = "0";
...
function get_customers_state_filter($zone_id) {
	switch($zone_id) {
		case 182:
			$query = " and ( (a.entry_postcode <= 4999 and a.entry_postcode >= 4000) OR (a.entry_postcode <= 9999 and a.entry_postcode >= 9000) OR (a.entry_zone_id=182) )  ";		
			break;
		case 184:
			$query = " and ( (a.entry_postcode <= 1999 and a.entry_postcode >=1000) OR (a.entry_postcode <= 2599 and a.entry_postcode >= 2000) OR (a.entry_postcode <= 2898 and a.entry_postcode >= 2619) OR (a.entry_postcode <= 2999 and a.entry_postcode >= 2921) OR (a.entry_zone_id=184) )  ";	
			break;
		case 185:
			$query = " and ( (a.entry_postcode <= 3999 and a.entry_postcode >= 3000) OR (a.entry_postcode <= 8999 and a.entry_postcode >= 8000) OR (a.entry_zone_id=185) )  ";		
			break;
		case 186:
			$query = " and ( (a.entry_postcode <= 5799 and a.entry_postcode >= 5000) OR (a.entry_postcode <= 5999 and a.entry_postcode >= 5800) OR (a.entry_zone_id=186) )  ";		
			break;
		case 187:
			$query = " and ( (a.entry_postcode <= 6797 and a.entry_postcode >= 6000) OR (a.entry_postcode <= 6999 and a.entry_postcode >= 6800) OR (a.entry_zone_id=187) )  ";		
			break;
		case 188:
			$query = " and ( (a.entry_postcode <= 299 and a.entry_postcode >= 200) OR (a.entry_postcode <= 2618 and a.entry_postcode >= 2600) OR (a.entry_postcode <= 2920 and a.entry_postcode >= 2900) OR (a.entry_zone_id=188) )  ";	
			break;
		case 189:
			$query = " and ( (a.entry_postcode <= 899 and a.entry_postcode >= 800) OR (a.entry_postcode <= 999 and a.entry_postcode >= 900) OR (a.entry_zone_id=189) )  ";		
			break;
		case 190:
			$query = " and ( (a.entry_postcode <= 7799 and a.entry_postcode >= 7000) OR (a.entry_postcode <= 7999 and a.entry_postcode >= 7800) OR (a.entry_zone_id=190) )  ";		
			break;
		default:
			$query = " ";		
			break;
...
//get in active customers by state
function tep_get_inactive_customers_by_state($zone_id) {
	$query_string = get_customers_state_filter($zone_id);	
	
	//echo "SELECT distinct c.customers_id FROM customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id) WHERE c.customers_default_address_id = a.address_book_id AND c.customers_orders_count = 0 AND a.entry_country_id='13' ".$query_string. "GROUP BY c.customers_id";
	//echo "<br>";
	
	$customer_query = tep_db_query("SELECT distinct c.customers_id FROM customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id) WHERE c.customers_default_address_id = a.address_book_id AND c.customers_orders_count = 0 AND a.entry_country_id='13' ".$query_string . "GROUP BY c.customers_id");		
	return tep_db_num_rows($customer_query);
}

//get active customers by state
function tep_get_active_customers_by_state($zone_id) {
	$query_string = get_customers_state_filter($zone_id);	
	$customer_query = tep_db_query("SELECT distinct c.customers_id FROM customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id ) WHERE  c.customers_default_address_id = a.address_book_id AND c.customers_orders_count > 0 AND a.entry_country_id='13' ".$query_string . "GROUP BY c.customers_id");
	return tep_db_num_rows($customer_query);
}

/**************** Other city with in australia state *******************/
//get in active customers by state
function tep_get_inactive_customers_aus_other() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id from customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id AND c.customers_default_address_id = a.address_book_id) WHERE c.customers_orders_count = 0 AND a.entry_country_id='13' AND a.entry_zone_id NOT IN(182,184,185,186,187,188,189,190) ");
	return tep_db_num_rows($customer_query);
}

//get active customers other city within australia
function tep_get_active_customers_aus_other() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id from customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id AND c.customers_default_address_id = a.address_book_id) WHERE c.customers_orders_count > 0 AND a.entry_country_id='13' AND a.entry_zone_id NOT IN(182,184,185,186,187,188,189,190) ");
	return tep_db_num_rows($customer_query);
}

/**************** Other city with in australia statr *******************/

//get in active customers outside australia
function tep_get_inactive_customers_outside_aus() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id from customers c LEFT JOIN address_book a ON (c.customers_id=a.customers_id AND c.customers_default_address_id = a.address_book_id) WHERE c.customers_orders_count = 0 AND a.entry_country_id!='13' ");	
	return tep_db_num_rows($customer_query);
}

//get active customers outside australia
function tep_get_active_customers_outside_aus() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id from customers c LEFT JOIN address_book a ON (c.customers_id=a.customers_id AND c.customers_default_address_id = a.address_book_id) WHERE c.customers_orders_count > 0 AND a.entry_country_id!='13' ");	
	return tep_db_num_rows($customer_query);
}

//get all active customers
function tep_get_active_customers() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id FROM customers c WHERE c.customers_orders_count > 0 ");
	return tep_db_num_rows($customer_query);
}

//get all in active customers
function tep_get_inactive_customers() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id FROM customers c WHERE c.customers_orders_count = 0 ");
	return tep_db_num_rows($customer_query);
}

//get active customers this week
function tep_get_active_customers_this_week() {
	$timestamp = time();
	$last_sunday = date("Y-m-d H:i:s", strtotime("last sunday", $timestamp));
	$next_sunday = date("Y-m-d H:i:s", strtotime("next sunday", $timestamp));  
	$customer_query = tep_db_query("SELECT DISTINCT c.customers_id FROM customers c LEFT JOIN customers_info ci ON c.customers_id = ci.customers_info_id WHERE ci.customers_info_date_account_created > '".$last_sunday."' AND ci.customers_info_date_account_created < '".$next_sunday."' AND c.customers_orders_count > 0");
	return tep_db_num_rows($customer_query);
}

//get in active customers this week
function tep_get_inactive_customers_this_week() {
	$timestamp = time();
	$last_sunday = date("Y-m-d H:i:s", strtotime("last sunday", $timestamp));
	$next_sunday = date("Y-m-d H:i:s", strtotime("next sunday", $timestamp)); 	
	$customer_query = tep_db_query("SELECT DISTINCT c.customers_id FROM customers c LEFT JOIN customers_info ci ON c.customers_id = ci.customers_info_id WHERE ci.customers_info_date_account_created > '".$last_sunday."' AND ci.customers_info_date_account_created < '".$next_sunday."' AND c.customers_orders_count = 0");
...
//get  male customers by state
function tep_get_male_customers_by_state($zone_id) {
	$query_string = get_customers_state_filter($zone_id);	
	$customer_query = tep_db_query("SELECT distinct c.customers_id FROM customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id ) WHERE  c.customers_default_address_id = a.address_book_id AND c.customers_gender='m' AND a.entry_country_id='13' ".$query_string . "GROUP BY c.customers_id");
	return tep_db_num_rows($customer_query);
}

//get  female customers by state
function tep_get_female_customers_by_state($zone_id) {
	$query_string = get_customers_state_filter($zone_id);	
	$customer_query = tep_db_query("SELECT distinct c.customers_id FROM customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id ) WHERE  c.customers_default_address_id = a.address_book_id AND c.customers_gender='f' AND a.entry_country_id='13' ".$query_string . "GROUP BY c.customers_id");
	return tep_db_num_rows($customer_query);
}

//get male customers outside australia
function tep_get_male_customers_outside_aus() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id from customers c LEFT JOIN address_book a ON (c.customers_id=a.customers_id AND c.customers_default_address_id = a.address_book_id) WHERE c.customers_gender = 'm' AND a.entry_country_id!='13' ");	
	return tep_db_num_rows($customer_query);
}

//get female customers outside australia
function tep_get_female_customers_outside_aus() {
	$customer_query = tep_db_query("SELECT distinct c.customers_id from customers c LEFT JOIN address_book a ON (c.customers_id=a.customers_id AND c.customers_default_address_id = a.address_book_id) WHERE c.customers_gender = 'f' AND a.entry_country_id!='13' ");	
	return tep_db_num_rows($customer_query);
}


//get customers zone id
function tep_get_customers_zone($customers_id) {	
	$customer_query = tep_db_query("SELECT a.entry_zone_id FROM customers c LEFT JOIN address_book a ON (c.customers_id = a.customers_id) WHERE c.customers_default_address_id = a.address_book_id AND c.customers_id = '".$customers_id."'");		
	$customers_arr = tep_db_fetch_array($customer_query);
	return $customers_arr['entry_zone_id'];
}

//get zone name by zone id
function tep_get_zonename($zone) {	
	$zone_query = tep_db_query("SELECT zone_name FROM zones WHERE zone_id = '".$zone."'");		
	$zone_arr = tep_db_fetch_array($zone_query);
	return $zone_arr['zone_name'];
}


//get customers revenue by state and duration
function tep_get_all_products_model() {
	
	$products_query = tep_db_query("SELECT DISTINCT products_model FROM `products` group by products_model");
	...
	if(tep_db_num_rows($products_query)>0) {
		...
		$models_color = array("#33AD5B","#FF7900","#4C8BFF","#9CD988","#B36912","#BC002F","#FCF800","#FFB760","#E501EC","#69A55C","#F4CCCB","#88CCEE","#C72C95","#03DD23","#0903AD","#FFEDB3","#D4B385","#9E9B95","#729490","#546B24","#FF0000");
	
		while($products_arr = tep_db_fetch_array($products_query)) {
						
			$products_code = explode("-",$products_arr["products_model"]);				
			if($products_code[0]=="" || $products_code[0]<1) { $pmodel = '000'; }
			else { $pmodel = $products_code[0]; }				
			$model_arr[$pmodel] = $pmodel; 				
		}		
				
		//$model_arr = array_combine($model_arr, $models_color);
		$i=0;
		foreach($model_arr as $models) {
			$models_arr[$models] = $models_color[$i];
			$i++;
		}
	}
...
//get customers account per sales consultant
function tep_get_sales_consultant_accounts($sales_consultant) {	
	$customer_query = tep_db_query("SELECT c.customers_id FROM customers c WHERE c.customers_orders_count > 0 AND c.sales_consultant = '".$sales_consultant."'");		
	$customers_row = tep_db_num_rows($customer_query);
	return $customers_row;
}

//get customers account per sales consultant per state
function tep_get_sales_consultant_accounts_per_state($sales_consultant) {	
	$customer_query = tep_db_query("SELECT c.customers_id, a.entry_zone_id, a.entry_postcode FROM customers c, address_book a WHERE c.customers_id=a.customers_id AND c.customers_orders_count > 0 AND c.sales_consultant = '".$sales_consultant."'");	
	while($cust_arr = tep_db_fetch_array($customer_query)) {		
		//$zoneid = $cust_arr["entry_zone_id"];
		$zoneid = get_zone_by_post_code($cust_arr["entry_postcode"]);
		if($zoneid=="") { $zoneid = 0; }
		$customer[$zoneid]++;
	}		
	return $customer;
...
//July 02 2012
function tep_get_customer($cID) {
	$customer_query = tep_db_query("select c.*, a.*, ci.* from customers c left join address_book a on c.customers_default_address_id = a.address_book_id LEFT JOIN customers_info ci on c.customers_id=ci.customers_info_id where a.customers_id = c.customers_id and c.customers_id = '" . (int)$cID . "'");
    $customer_info = tep_db_fetch_array($customer_query);

    return $customer_info;
}

function tep_update_duedate($oID) {

	$ord_qry = tep_db_query("select customers_id from orders where orders_id='".$oID."'");
	$ord_arr = tep_db_fetch_array($ord_qry);
	$cust_arr = tep_get_customer($ord_arr['customers_id']);
	
	$duedate = date("Y-m-d H:i:s");
	
	if(!empty($cust_arr['customers_term'])) {
		$cust_term = preg_replace('/\D/', '', $cust_arr['customers_term']);
		if(is_numeric($cust_term)) {
			$duedate = date("Y-m-d H:i:s", strtotime('+'.$cust_term.' days'));		
		}		
	}	
	$ord_qry = tep_db_query("UPDATE orders SET due_date='".$duedate."' where orders_id='".$oID."'");
}

//Aug 15 2012

//Stock updates for badge products :: QUICK UPDATES
function tep_update_products_stock($id,$quantity,$status,$languages_id) {
	
	$stock_query = tep_db_query("select products_quantity, overall_quantity, progressed_quantity, products_parent_id, default_product_id from " . TABLE_PRODUCTS . " where products_id = '" . $id . "'");	  
	$stock_values = tep_db_fetch_array($stock_query);

	//For update the Parent Product Quantity
	if ($stock_values['products_parent_id'] != 0 || $stock_values['default_product_id']!=0) {

		$product_id = $stock_values['products_parent_id'];
								
		if($product_id==0 || $product_id=="") {
			$product_id = $stock_values['default_product_id'];
		}
		
	} else {
		$product_id = $id;
	}
	$parent_query = tep_db_query("select products_quantity, overall_quantity, progressed_quantity from " . TABLE_PRODUCTS . " where products_id = '" . tep_get_prid($product_id) . "'");
	$parent_values = tep_db_fetch_array($parent_query);
	
	if($status==100006) {	
		$parent_product_stock_left = $parent_values['overall_quantity'];
		if($parent_values['overall_quantity']>=$quantity) {
			$parent_product_stock_left = $parent_values['overall_quantity'] - $quantity;
		}
		
		$parent_progressed_quantity = $parent_values['progressed_quantity'];
		if($parent_values['progressed_quantity']>=$quantity) {
			$parent_progressed_quantity = $parent_values['progressed_quantity'] - $quantity;
		}
		$upd_prd = tep_db_query("update " . TABLE_PRODUCTS . " set overall_quantity = '" . $parent_product_stock_left . "', progressed_quantity = '".$parent_progressed_quantity."' where products_id = '" . tep_get_prid($product_id) . "'");
	} 
	else if($status==100008 || $status==6) {
		$parent_product_stock_left = $parent_values['products_quantity'] + $quantity;
		$parent_progressed_quantity = $parent_values['progressed_quantity'];
		if($parent_values['progressed_quantity']>=$quantity) {
			$parent_progressed_quantity = $parent_values['progressed_quantity'] - $quantity;
		}
		$upd_prd = tep_db_query("update " . TABLE_PRODUCTS . " set products_quantity = '" . $parent_product_stock_left . "', progressed_quantity = '".$parent_progressed_quantity."' where products_id = '" . tep_get_prid($product_id) . "'");
	}
	
	if($upd_prd) { return true; }
	else { return false; }
}

//Stock updates for badge products attributes :: QUICK UPDATES
function tep_update_products_attributes_stock($id,$quantity,$type,$status,$languages_id) {
	
	if($type=="badge") {
	
		$prd_desc_qry = tep_db_query("SELECT options_values_id FROM products_badge_options WHERE products_id='".$id."'");
		$option_values = tep_db_fetch_array($prd_desc_qry);		
		$option_values_id = $option_values["options_values_id"];
	
	} else {
		$option_values_id = $id;
	}
	
	$pov_query = tep_db_query("SELECT * FROM ".TABLE_PRODUCTS_OPTIONS_VALUES." WHERE products_options_values_id='".$option_values_id."'");	  
	$pov_array = tep_db_fetch_array($pov_query);	  
	  
	if($status==100006) {		
		//orders processed
		$stock_left = $pov_array['overall_quantity'];
		if($pov_array['overall_quantity']>=$quantity) {
			$stock_left = $pov_array['overall_quantity'] - $quantity ;      
		}
		
		$stock_progressed = $pov_array['progressed_quantity'];
		if($pov_array['progressed_quantity']>=$quantity) {
			$stock_progressed = $pov_array['progressed_quantity'] - $quantity;
		}
		$upd = tep_db_query("update " . TABLE_PRODUCTS_OPTIONS_VALUES . " set overall_quantity = '" . $stock_left . "' , progressed_quantity = '".$stock_progressed."' where products_options_values_id = '" .$option_values_id . "'");
	} 
	else if($status==100008 || $status==6) {
		//orders cancelled, so restock
		$stock_left = $pov_array['quantity'] + $quantity;      
		$stock_progressed = $pov_array['progressed_quantity'];
		if($pov_array['progressed_quantity']>=$quantity) {
			$stock_progressed = $pov_array['progressed_quantity'] - $quantity;
		}
		$upd = tep_db_query("UPDATE " . TABLE_PRODUCTS_OPTIONS_VALUES . " SET quantity = '" . $stock_left . "' , progressed_quantity = '".$stock_progressed."' where products_options_values_id = '" .$option_values_id . "'");
...
//Aug 16 2012
function tep_get_invoice_date($oID) {
	$orders_history_qry = tep_db_query("SELECT MIN(date_added) as invoice_date FROM orders_status_history WHERE orders_id='".$oID."' AND orders_status_id='100006'");
	if(tep_db_num_rows($orders_history_qry)>0) {
		$arr = tep_db_fetch_array($orders_history_qry);
		return $arr["invoice_date"];
	} else {
		return false;
	}
}

//Badge settings functions
function tep_is_badge_settings_exists($products_id) {
    $qry = tep_db_query("SELECT products_id FROM products_badge_settings WHERE products_id='".$products_id."'");
    if(tep_db_num_rows($qry)>0) {
        return true;
    } else { return false; }
}

function tep_get_badge_settings($products_id) {
    $qry = tep_db_query("SELECT * FROM products_badge_settings WHERE products_id='".$products_id."'");
    if(tep_db_num_rows($qry)>0) {
        return tep_db_fetch_array($qry);
    } else { return false; }
}
//Badge settings functions
//get badge products attributes options - Dec 21 2012
function tep_get_badge_products_options($products_id) {
	$desc = "";
	$qry = tep_db_query("SELECT * FROM products_badge_options WHERE products_id='".$products_id."'");
	if(tep_db_num_rows($qry) > 0) {
		while($arr = tep_db_fetch_array($qry)) {
			$desc .= $arr["options_desc"]."<br>";
		}		
		$desc = "<b>Attributes&nbsp;&nbsp;&nbsp;</b>".$desc;
	}
	return $desc;
}

//get feedback email status
function tep_get_feedback_email_setting($customers_id) {
	$feed_back_query=tep_db_query("select send_feedback_email from customers WHERE customers_id='".$customers_id."'");
	$feed_back_value=tep_db_fetch_array($feed_back_query);				
	return $feed_back_value['send_feedback_email'];
}


function tep_get_artwork_link($orders_id) {
	$link = "";
	$artwork_qry = tep_db_query("SELECT DISTINCT customers_id, 
				   (SELECT COUNT(artwork_id) from artwork where orders_id='".$orders_id."' and artwork_status='pending') as pcnt,
				   (SELECT COUNT(artwork_id) from artwork where orders_id='".$orders_id."' and artwork_status='revision') as rcnt,
				   (SELECT COUNT(artwork_id) from artwork where orders_id='".$orders_id."' and (artwork_status='approve' OR artwork_status='approved')) as acnt from artwork where orders_id='".$orders_id."'");
											
		if(tep_db_num_rows($artwork_qry)>0) { 
			
			$artwork_arr = tep_db_fetch_array($artwork_qry);							
			 
			 if($artwork_arr['pcnt']>0) {
				$design_img = "artwork-pending.png";
			 } else if($artwork_arr['rcnt']>0) {
				$design_img = "artwork-revision.png";
			 } else if($artwork_arr['acnt']>0) {
				$design_img = "artwork-approved.png";
			 }
			
			$link = '<a href="' . tep_href_link("artworks.php", 'cID=' . $artwork_arr['customers_id'], 'SSL') . '">' . tep_image(DIR_WS_IMAGES . "artwork-icon.png", "Artwork") . tep_image(DIR_WS_IMAGES . $design_img, "Artwork") . '</a>';
		}
	return $link;	
}

//get count of customers artworks by order
function tep_get_customers_artworks($cID,$oID) {
	//echo "SELECT count(*) as total_artworks FROM artwork WHERE customers_id = '".$cID."' AND orders_id='".$oID."'";
	
	if($oID>0) {
		$qry = tep_db_query("SELECT count(*) as total_artworks FROM artwork WHERE customers_id = '".$cID."' AND orders_id='".$oID."'");
	} else {
		$qry = tep_db_query("SELECT count(*) as total_artworks FROM artwork WHERE customers_id = '".$cID."'");
	}
	
	$arr = tep_db_fetch_array($qry);

	return $arr["total_artworks"];
}			

function tep_get_customers_myfiles($cID) {
	$qry = tep_db_query("select count(*) as total_myfiles from ((SELECT o.orders_id, o.date_purchased, p.badge_data, o.customers_name, o.customers_company from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_PRODUCTS . " p where o.customers_id = '" . (int)$cID . "' && o.orders_id = op.orders_id && op.products_id = p.products_id && p.badge_data!='') union all (select files_id, date_uploaded, file_name, file_path, comment from my_files where customers_id='". (int)$cID ."')) as tmp order by date_purchased desc");
	$arr = tep_db_fetch_array($qry);
	return $arr["total_myfiles"];
}

function tep_get_orders_sales_consultant($oID) {
	$qry = tep_db_query("SELECT order_assigned_to_email, order_assigned_to FROM orders WHERE orders_id = '".$oID."'");
	if(tep_db_num_rows($qry)>0) {
		$arr = tep_db_fetch_array($qry);
		return $arr;
	} else { return false; }
...
function tep_get_orders_total($oID) {
	$rst = tep_db_query("select ot.text as order_total from " . TABLE_ORDERS_TOTAL . " ot where  ot.orders_id = '" . $oID . "' and ot.class = 'ot_total'");	  
	$arr = tep_db_fetch_array($rst);
	$total = $arr['order_total'];
	if (!$total){ $total = TEXT_INFO_ABANDONDED; }
	return $total;	  
}
 
function tep_get_orders_shipping($oID) {
	$rst = tep_db_query("select ot.title as shipping from " . TABLE_ORDERS_TOTAL . " ot where  ot.orders_id = '" . $oID . "' and ot.class ='ot_shipping'");	  
	$arr = tep_db_fetch_array($rst);
	$ship = "";	
	
	if (preg_match("/Same Day Service/i", $arr['shipping'])) {
		$ship = "Same Day";
	} else if(preg_match("/Next Day Dispatch/i", $arr['shipping'])) {
		$ship = "Next Day";
	}	
	return $ship;	  
}
