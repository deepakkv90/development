              ...
              for ($i=0; $i < sizeof($languages); $i++) {
                ...
                <div class="tab-page" id="<?php echo $languages[$i]['name'];?>">
                  ...
                  <table width="100%"  border="0" cellspacing="0" cellpadding="0" summary="tab table">
                    ...
                    <tr>
                      <td valign="top"><fieldset style="display:none;"> <legend>&nbsp;</legend></fieldset>
					  <fieldset>
                        ...
                        <?php
                          ...
                          <fieldset><legend><?php echo TEXT_AUXILIARY_CONTENT_FILE; ?></legend>
                            <table width="100%" cellspacing="0" cellpadding="2" border="0">
                              <tr>               
                                <?php
                               $file_name = DIR_FS_DOCUMENT_ROOT . DIR_WS_LANGUAGES . $languages[$i]['directory'] . '/pages/' . tep_pages_get_auxillary_file($page_id, $languages[$i]['id']);
                                $file_location =  DIR_WS_LANGUAGES . $languages[$i]['directory'] . '/pages/';
                                if ( tep_pages_get_auxillary_file($page_id, $languages[$i]['id']) == '' )  {
                                  $cds_aux_file = 'Upload files';
                                } else if (file_exists($file_name) ) {
                                  $acf_file_exists = true;
                                  $cds_aux_file = '<a href="javascript:popUp(\'' . tep_href_link(FILENAME_CDS_POPUP,'action=view'.'&pID='.(int)$pID.'&language_id='.$languages[$i]['id']) . '\')">' . tep_image(DIR_WS_IMAGES . '/icons/preview.gif', TEXT_PREVIEW) . '<b>' . tep_pages_get_auxillary_file($page_id, $languages[$i]['id']) .'</b></a>';
                                } else {
                                  $cds_aux_file = '<span class="errorText">' . sprintf(TEXT_ACF_FILE_MISSING,tep_pages_get_auxillary_file($page_id, $languages[$i]['id']) ).'</span><br><span class="smallText">' . TEXT_ACF_FILE_PLEASE_UPLOAD . '<b>' . $file_location . '</b></span>';
                                }
                                echo '<td valign="middle" align="center" class ="main">'. $cds_aux_file .'</td>';     
                                $pID = isset($_GET['pID']) ? (int)$_GET['pID'] : '';
                                $pageid = 0;
                                $pageid = tep_pages_get_auxillary_file($page_id, $languages[$i]['id']); 
                                $pagename = "";
                                if ( $pageid != 0 ) {
