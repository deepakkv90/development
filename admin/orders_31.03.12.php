...
*/
 ...
 function manage_crm_order_status($oID,$status,$status_name)
 {
 	$ch = curl_init();
 ...
 //End Of  Function For updtae status of order in crm
    ...
    switch ($action) {
      ...
      case 'update_order':
       ...
       //cal soap function to update status in crm
       	//manage_crm_order_status($oID,$status,$status_name);
                  ...
                  <tr>
					...
					<?php 
						...
						if($order->customer['company']!="") {
							echo '<u><a href="' . tep_href_link(FILENAME_CUSTOMERS, 'cID=' . $order->customer['id'].'&action=edit', 'SSL') . '">' . $order->customer['company'] . '</a></u>' ."<br>";				
							echo $order->customer['name']."<br>";	
						} else {
							echo '<u><a href="' . tep_href_link(FILENAME_CUSTOMERS, 'cID=' . $order->customer['id'].'&action=edit', 'SSL') . '">' . $order->customer['name'] . '</a></u>' ."<br>";	
						}
...
if (defined('MVS_STATUS') && MVS_STATUS == 'true') {
  ...
  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
    ...
    if ($prod_img['badge_data']) {
      require_once(dirname(dirname(__FILE__)).'/templates/nbi_au/bd/badge_desc.php');
...
?>
      ...
      <tr>
        <td class="main"><table border="1" cellspacing="0" cellpadding="5">
            ...
            <?php
    $orders_history_query = tep_db_query("select orders_status_id, date_added, customer_notified, comments, admin_users_id from " . TABLE_ORDERS_STATUS_HISTORY . " where orders_id = '" . tep_db_input($oID) . "' order by date_added");
    if (tep_db_num_rows($orders_history_query)) {
      while ($orders_history = tep_db_fetch_array($orders_history_query)) {
        ...
        echo '            <td class="smallText">' . $orders_status_array[$orders_history['orders_status_id']] . '</td>' . "\n" .
			 '            <td class="smallText">' . $admin_info['admin_firstname'] . " " . $admin_info['admin_lastname'] . '</td>' . "\n" .
...
?>
    ...
    if (isset($_GET['cID'])) {
     ...
     $orders_query_raw = "select o.orders_id, o.customers_name, o.customers_email_address, o.customers_company, o.customers_id, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.orders_status, s.orders_status_name, ot.text as order_total from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot, " . TABLE_ORDERS_STATUS . " s where o.customers_id = '" . (int)$cID . "' and ot.orders_id = o.orders_id and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' and ot.class = 'ot_total' order by orders_id DESC";
    } elseif (isset($_GET['status']) && (tep_not_null($_GET['status']))) {
      $status = tep_db_prepare_input($_GET['status']);
      $orders_query_raw = "select o.orders_id, o.customers_name, o.customers_email_address, o.customers_company, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.orders_status, s.orders_status_name from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_STATUS . " s where o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' and s.orders_status_id = '" . (int)$status . "' order by o.orders_id DESC";
    } 
	...
	elseif(isset($_GET['sel_status'])) {
		...
		$orders_query_raw = "select o.orders_id, o.customers_name, o.customers_email_address, o.customers_company, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.orders_status, o.orders_status, s.orders_status_name from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_STATUS . " s where o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "'";
	...
	elseif (isset($_GET['SoID'])) {
      $SoID = tep_db_prepare_input($_GET['SoID']);
      $orders_query_raw = "SELECT o.orders_id, o.customers_name, o.customers_email_address, o.customers_company, o.customers_id, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.orders_status, s.orders_status_name
                           FROM " . TABLE_ORDERS . " o,
...
?>
    ...
    switch ($action) {
      case 'delete':
		...
		case 'pdf':
        //For Extra Customer notification - June 29 2011
      ...
      default:
        if (isset($oInfo) && is_object($oInfo)) {
		 ...
		 //Mar 09 2011 - New Link for Delete Order
