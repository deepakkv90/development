  ...
  switch ($action) {
    ...
    case 'update_category':
      if (isset($_POST[IMAGE_BACK]) || !isset($_POST['sort_order'])) {
        ...
        $sql_data_array = array('sort_order' => $sort_order, 'show_on_quote' => $show_on_quote);
        $customergroupid = "";
    ...
    <td class="page-container" valign="top">
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        ...
        if ( isset($action) && ($action == 'new_category' || $action == 'edit_category') ) {
          if ( ($cID) && (!$_POST) ) {
            $categories_query = tep_db_query("select c.categories_id, cd.categories_name, cd.categories_heading_title, cd.categories_description, cd.categories_head_title_tag, cd.categories_head_desc_tag, cd.categories_head_keywords_tag, c.categories_image, c.parent_id, c.sort_order, c.show_on_quote, c.date_added, c.last_modified,products_group_access, c.products_nav_access from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . $cID . "' and c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' order by c.sort_order, cd.categories_name");
              ...
              <td><fieldset style="display:none;"></fieldset><fieldset><legend><?php echo TEXT_GENERAL_OPTIONS; ?></legend>
                <table width="100%" border="0" cellspacing="3" cellpadding="3" align="center" summary="category image and sort order">
                  <tr valign="top">
                    ...
                    <td width="33%" class="main" align="center">
						<strong><?php echo TEXT_EDIT_SORT_ORDER; ?></strong>&nbsp;<?php echo tep_draw_input_field('sort_order', $cInfo->sort_order, 'size="2"'); ?><br/>
						<strong><?php echo TEXT_EDIT_SHOW_ON_QUOTE; ?></strong>&nbsp;
						<select name="show_on_quote" style="width:40px;"><option value="1" <?php if($cInfo->show_on_quote==1) { echo "selected"; }?> >Yes</option><option value="0" <?php if($cInfo->show_on_quote==0) { echo "selected"; }?> >No</option></select>
