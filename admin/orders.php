  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update_order':
		...
		$print_file_update = 0;
		$printfiles = $_POST["print_file"];		
		foreach($printfiles as $productid_key=>$eps_value) {
			if(!empty($eps_value)) {				
				tep_db_query("UPDATE orders_products SET print_file_name='".$eps_value."' WHERE orders_id = '".$oID."' AND products_id='".$productid_key."'");
				$print_file_update = 1;
			}
		}
		
		//Inventory Updation :: Aug 14 2012
		
		if($status==100006) {
			
			$status_qry = tep_db_query("SELECT orders_status_history_id FROM orders_status_history WHERE orders_id='".$oID."' AND orders_status_id='100006'");
			
			if(tep_db_num_rows($status_qry)==0) {			
				...
				for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
					...
					tep_update_products_stock(tep_get_prid($order->products[$i]['id']),$order->products[$i]['qty'],$status,$languages_id);

					if (isset($order->products[$i]['attributes']) && (sizeof($order->products[$i]['attributes']) > 0)) {
					  for ($j = 0, $k = sizeof($order->products[$i]['attributes']); $j < $k; $j++) {
																	
						tep_update_products_attributes_stock($order->products[$i]['attributes'][$j]['value_id'],$order->products[$i]['qty'],"nonbadge",$status,$languages_id);
						
					  }
					
					} else {
						
						  tep_update_products_attributes_stock($order->products[$i]['id'],$order->products[$i]['qty'],"badge",$status,$languages_id);
			...
			$status_qry = tep_db_query("SELECT orders_status_history_id FROM orders_status_history WHERE orders_id='".$oID."' AND (orders_status_id='100008' OR orders_status_id='6')");
			...
			if(tep_db_num_rows($status_qry)==0) {			
				...
				for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {										  
					...
					if (isset($order->products[$i]['attributes']) && (sizeof($order->products[$i]['attributes']) > 0)) {
					  //For Non badge products attributes
					  for ($j = 0, $k = sizeof($order->products[$i]['attributes']); $j < $k; $j++) {
							tep_update_products_attributes_stock($order->products[$i]['attributes'][$j]['value_id'],$order->products[$i]['qty'],"nonbadge",$status,$languages_id);						
					  }
					
					} else { 
						  //For Badge Products Attributes
						  tep_update_products_attributes_stock($order->products[$i]['id'],$order->products[$i]['qty'],"badge",$status,$languages_id);
					}
        ...
        $order_updated = false;
        $check_status_query = tep_db_query("SELECT customers_id, customers_name, customers_email_address, orders_status, date_purchased
                                            FROM " . TABLE_ORDERS . "
        ...
        if ( ($check_status['orders_status'] != $status) || tep_not_null($comments) || ($status == DOWNLOADS_ORDERS_STATUS_UPDATED_VALUE) ) {
          ...
          // multi-vendor shipping //eof          
			...
			//Notification to Admin User
			if (isset($_POST['notify_sales']) && ($_POST['notify_sales'] == 'on')) {				
				$sales_consultant = tep_get_orders_sales_consultant($oID);
				$admin_details = tep_get_admin_details((int)$_SESSION['login_id']);
				
				if(!empty($sales_consultant["order_assigned_to_email"])) {
					
					$email_sales = sprintf(EMAIL_GREET_TEXT, $sales_consultant['order_assigned_to'])."\n\n". 'Please check below messages for queries,'."\n\n".'<b>'.EMAIL_TEXT_ORDER_NUMBER . ' ' . $oID . "</b>\n\n Comments are: \n" . $comments."\n......\n".$admin_details["admin_firstname"]." ".$admin_details["admin_lastname"];
					
					tep_mail($sales_consultant["order_assigned_to"], $sales_consultant["order_assigned_to_email"], sprintf(EMAIL_NOTIFY_SALES_SUBJECT,$oID), $email_sales, $admin_details["admin_firstname"]." ".$admin_details["admin_lastname"], $admin_details["admin_email_address"]);
					
				} else { $comments .= "\n\n Note: Order notification failure. Sales Consultant Email Address not available."; }
			}
		...
		//Aug 30 2012
		if(isset($_POST['submit_xero']) || isset($_POST['paid']) || ($print_file_update==1)) { $order_updated = true; }
        ...
        if ($order_updated == true) {
		  ...
		  //Feedback email entry - Start
          if(isset($_POST['status']) && $_POST['status']==100006) {             
            //Check feedback email settings for the customer
			$fbsetting = tep_get_feedback_email_setting($check_status['customers_id']);
		    if($fbsetting=='1') {
				
                $date_scheduled = "";
                                
                $fb_chk = tep_db_query("SELECT MAX(date_scheduled) as recent_date FROM feedback_info WHERE customers_id='".$check_status['customers_id']."' AND date_scheduled > NOW() - INTERVAL 1 WEEK");                
                $fb_chk_arr = tep_db_fetch_array($fb_chk);                                      
                if(!empty($fb_chk_arr["recent_date"])) {                    
                        $next_month = strtotime($fb_chk_arr["recent_date"]." +1 week");
                        $date_scheduled = date("Y-m-d H:i:s",$next_month);
                }                
                
                if($date_scheduled=="") {                   
                    $weeks = strtotime("+1 week");  
                    $date_scheduled = date("Y-m-d H:i:s",$weeks);
                }
                
                $fb_qry = tep_db_query("SELECT feedback_info_id FROM feedback_info WHERE orders_id = '" . (int)$oID . "' AND customers_id='".$check_status['customers_id']."'");
                if(tep_db_num_rows($fb_qry)==0) {
                    $fb_arr = tep_db_fetch_array($fb_qry);
                    $date_invoiced = date("Y-m-d H:i:s");                    
                    $fb_ins = tep_db_query("INSERT INTO feedback_info SET orders_id = '" . (int)$oID . "', customers_id='".$check_status['customers_id']."', status='0', date_invoiced='".$date_invoiced."', date_scheduled='".$date_scheduled."'");
                }               
			}	
          }
          //Feedback email entry - End
	...
	$(document).ready(function() {
		...
		$("input[name='submit_xero']").click(function(){
			if($(this).is(":checked")) {							
				...
				$("#smt_final_invoice").show();
			} else {
								
				$("#smt_order_update").hide();
				$("#smt_final_invoice").hide();
		...
		//Final invoice alert
		$("#smt_final_invoice").click(function() {
			var r=confirm("Are you sure you want to close this invoice. \n You will not be able to edit or change anything after.\n\n The order will be listed with invoice status in the order listing.\n\n This process is irreversible.");
			if (r==true) {
				$("#status_options").val("100006");
				$("#frm_status").submit();
			}
		});
		
		$("#smt_order_update").click(function() {
			
			if($("#status_options").val()=="100006") {
				var r=confirm("Are you sure you want to close this invoice. \n You will not be able to edit or change anything after.\n\n The order will be listed with invoice status in the order listing.\n\n This process is irreversible.");
				if (r==true) {					
					return true;
					//$("#frm_status").submit();					
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="2">
			...
			<tr>				
				<td colspan="3">
					<?php
						$cinfo = tep_get_customer($order->customer['customer_number']);
						$art_link = tep_get_artwork_link($oID);
					?>
					<table align="center" width="65%" cellspacing="0" cellpadding="2">
						<tr class="acc-header">
							<td class="bottom-ln" align="center"><a href="<?php echo tep_href_link(FILENAME_CUSTOMERS, 'cID=' . $order->customer['id'].'&custnum='.$order->customer['id'], 'SSL'); ?>"><?php echo CUSTOMERNO_TITLE . '&nbsp;' . tep_image(DIR_WS_ICONS . 'magnifier.png', ICON_PREVIEW); ?></a></td>
							<td class="bottom-ln left-ln" align="center"><?php echo ACCOUNTCREATED_TITLE; ?></td>
							<td class="bottom-ln left-ln" align="center"><a href="<?php echo tep_href_link(FILENAME_ORDERS, 'cID=' . $order->customer['customer_number']); ?>"><?php echo ORDER_TITLE; ?></a></td>
							<td class="bottom-ln left-ln" align="center" style="background:url('../images/myfile_small.png') no-repeat 5% center; padding:5px;"><a href="<?php echo tep_href_link(FILENAME_CUSTOMER_FILES, 'cID=' . $order->customer['customer_number'], 'SSL'); ?>"><?php echo MY_FILES_TITLE; ?></a></td>
							<td class="bottom-ln left-ln" align="center" style="background:url('../images/artwork_small.png') no-repeat 5% center; padding:5px;"><a href="<?php echo tep_href_link("artworks.php", 'cID=' . $order->customer['customer_number'], 'SSL'); ?>"><?php echo MY_ARTWORK_TITLE . " (".tep_get_customers_artworks($order->customer['customer_number'],0).") "; ?></a></td>				
						</tr>
						<tr >
							<td align="center"><a href="<?php echo tep_href_link(FILENAME_CUSTOMERS, 'cID=' . $order->customer['id'].'&custnum='.$order->customer['id'], 'SSL'); ?>"><?php echo $order->customer['customer_number']; ?></a></td>
							<td class="left-ln" align="center"><?php echo tep_date_aus_format($cinfo["customers_info_date_account_created"],"long",$languages_id); ?></td>
							<td class="left-ln" align="center"><a href="<?php echo tep_href_link(FILENAME_ORDERS, 'cID=' . $order->customer['customer_number']); ?>"><?php echo $cinfo["customers_orders_count"]; ?></a></td>
							<td class="left-ln" align="center"><a href="<?php echo tep_href_link(FILENAME_CUSTOMER_FILES, 'cID=' . $order->customer['customer_number'], 'SSL'); ?>"><?php //echo tep_get_customers_myfiles($order->customer['customer_number']); ?></a></td>
							<td class="left-ln" align="center"><a href="<?php echo tep_href_link("artworks.php", 'cID=' . $order->customer['customer_number'], 'SSL'); ?>"><?php echo tep_get_customers_artworks($order->customer['customer_number'], $oID); ?></a><br/>
							<?php
							if(!empty($art_link)) {
								echo $art_link;
							}
            ...
            <tr>
              <td valign="top" width="33%"><table width="100%" border="0" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="main" valign="top" width="38%"><b><?php echo '<u><a href="' . tep_href_link(FILENAME_CUSTOMERS, 'cID=' . $order->customer['id'].'&custnum='.$order->customer['id'], 'SSL') . '">' . ENTRY_CUSTOMER . '&nbsp;' . '</a></u>'; ?></b></td>
                    <td class="main">
					<?php 
...
?>
				...
				<tr>
					<td width="50%" style="vertical-align:top;">
					  ...
					  <table border="0" cellspacing="0" cellpadding="2">
						  ...
						  <tr>
							<td class="main"><b><?php echo "Sales Consultant Email: "; ?></b></td>
							<td class="main"><?php echo $order->info['order_assigned_to_email']; ?></td>
						  </tr>
						  <?php } ?>
						  ...
						  <!-- add purchase number # // -->
						  <tr>
							  <td class="main"><b><?php echo "Purchase Number:"; ?></b></td>
							   <td class="main"><?php echo $order->customer['purchase_number']; ?></td>
						  </tr>
								
						  <!-- add date/time // -->
						  <tr>
							  <td class="main"><b><?php echo ORDER_DATE_TIME; ?></b></td>
						...
						<tr>
						  ...
						  <td class="main"><a href="<?php echo DIR_WS_CATALOG . 'images/zip_bages/order_'.$oID.'.zip'; ?>"> <img src="../images/zipped_package.png"/></a></td>
						</tr>
						<?php } ?>
	  ...
	  <?php echo tep_draw_form('status', FILENAME_ORDERS, tep_get_all_get_params(array('action')) . 'action=update_order', 'post', 'id="frm_status"', 'SSL'); ?>
...
if (defined('MVS_STATUS') && MVS_STATUS == 'true') {
  ...
  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
    ...
    echo '          <tr class="dataTableRow">' . "\n" .
		...
		echo '<br/>Product #: '. (int)$order->products[$i]['id']."<br/>";
    ...
    if ($prod_img['badge_data']) {
      require_once(dirname(dirname(__FILE__)).'/templates/nbi_resp/bd/badge_desc.php');
	...
	//Print file addition Start
	echo '<div class="print-file"><b>Print File:</b><br/>';						
	echo tep_draw_input_field('print_file['.$order->products[$i]['id'].']', $order->products[$i]['print_file_name'], 'style="width:95%; border:1px solid #999; background-color:#f2f2f2; padding:3px;"');
	echo '</div>';
	////Print file addition end
						  ...
						  <td align="center" colspan="5">
								...
								if($order->info["price_break_amount"]>0) {
									...
									<p style="font-weight:bold;">
										<?php echo STORE_NAME; ?> Price Break Discount : you save $<?php echo number_format($order->info["price_break_amount"],2); ?>	<br><br></p>
								<?php } ?>
						  </td>
			  ...
			  <tr>
				<td><table border="0" cellspacing="0" cellpadding="2">
					<tr>
					  <td><table border="0" cellspacing="0" cellpadding="2">
						  <tr>
							<td class="main"><b><?php echo ENTRY_STATUS; ?></b> <?php echo tep_draw_pull_down_menu('status', $orders_statuses, $order->info['orders_status_number'],'id="status_options"'); ?></td>
						  </tr>
						  <tr>
							<td class="main"><b><?php echo ENTRY_NOTIFY_CUSTOMER; ?></b> <?php echo tep_draw_checkbox_field('notify', '', false); ?></td>
							<td class="main"><b><?php echo ENTRY_NOTIFY_COMMENTS; ?></b> <?php echo tep_draw_checkbox_field('notify_comments', '', true); ?></td>
							<td class="main"><b><?php echo ENTRY_NOTIFY_SALES_CONSULTANT;?></b><?php echo tep_draw_checkbox_field('notify_sales', '', false);?></td>
						  </tr>
      ...
      <tr>
        ...
        <td colspan="2" align="right">
			...
			<?php
				...
				echo '<a href="javascript:popupWindow(\'' . (HTTP_SERVER . DIR_WS_ADMIN . "bottom_sticker_invoice.php") . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_invoice.gif', "Bottom sticker invoice") . '</a>';                
         
				echo '<a href="javascript:popupWindow(\'' . (HTTP_SERVER . DIR_WS_ADMIN . "sticker_label_invoice.php") . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_invoice.gif', "Shipping sticker label") . '</a>';                
				
				echo tep_image_submit('button_update.gif', "Final Invoice", $hide_button. 'id="smt_final_invoice" '); 
			?>
			<br />		
			<?php 
				echo '<a href="' . tep_href_link(FILENAME_PAYPAL, tep_get_all_get_params(array('action')), 'SSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><a href="javascript:popupWindow(\'' . (HTTP_SERVER . DIR_WS_ADMIN . FILENAME_ORDERS_INVOICE) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a><a href="javascript:popupWindow(\'' . (HTTP_SERVER . DIR_WS_ADMIN . FILENAME_ORDERS_PACKINGSLIP) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>' . $buttons_bottom; 
			?>
		</td>
        <?php
        } else { //not paypal
        ...
        <td colspan="2" align="right">
			...
			<br />		
			<?php 
				echo '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action')), 'SSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><a href="javascript:popupWindow(\'' . (HTTP_SERVER . DIR_WS_ADMIN . FILENAME_ORDERS_INVOICE) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a><a href="javascript:popupWindow(\'' . (HTTP_SERVER . DIR_WS_ADMIN . FILENAME_ORDERS_PACKINGSLIP) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>' . $buttons_bottom; 
			?>
		</td>
        <?php
...
?>
                  <tr class="dataTableHeadingRow">
                    <td class="dataTableHeadingContent" width="6%"><?php echo TABLE_HEADING_ORDERID; ?></td>
                    <td class="dataTableHeadingContent" width="12%"><?php echo $HEADING_CUSTOMERS; ?></td>
                    ...
                    <td class="dataTableHeadingContent" width="12%" align="center"><?php echo $HEADING_DATE_PURCHASED; ?></td>
					<td class="dataTableHeadingContent" align="center" width="6%">Shipping</td>
					<td class="dataTableHeadingContent" align="center" width="3%">&nbsp;</td>
    ...
    if (isset($_GET['cID'])) {
     ...
     $orders_query_raw = "select o.orders_id, o.customers_name, o.customers_email_address, o.customers_company, o.customers_id, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.orders_status, o.xero, s.orders_status_name, ot.text as order_total from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot, " . TABLE_ORDERS_STATUS . " s where o.order_display='1' and o.customers_id = '" . (int)$cID . "' and ot.orders_id = o.orders_id and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' and ot.class = 'ot_total' order by orders_id DESC";
      ...
      $orders_query_raw = "select o.orders_id, o.customers_name, o.customers_email_address, o.customers_company, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, o.orders_status, o.xero, s.orders_status_name from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_STATUS . " s where o.order_display='1' and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' and s.orders_status_id = '" . (int)$status . "' order by o.orders_id ASC";
    } 
	...
	elseif(isset($_GET['sel_status'])) {
		...
		$orders_query_raw .= " order by o.orders_id ASC";
	} 
	...
	while ($orders = tep_db_fetch_array($orders_query)) {
      ...
      $order_total1 = tep_get_orders_total($orders['orders_id']);
      $ship_method = tep_get_orders_shipping($orders['orders_id']);
...
?>
					...
					<td class="dataTableContent" align="center"><?php echo $ship_method; ?></td>
