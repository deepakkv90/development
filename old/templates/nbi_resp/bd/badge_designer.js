...
jQuery.fn.badgeDesigner = function(options) {
    ...
    jQuery.extend(designer, {
        ...
        UpdatePreview: function() {
            ...
            //$('#preview_image').css('display', false);
    ...
    //setInterval(function() { designer.UpdatePreview(); },3000);
    ...
    for (i = 0; i < options.shapes.length; i++) {
        if (!designer.currentShape) {
            designer.currentShape = options.shapes[i];
        }
    ...
    if (options.badge_data) {
        ...
        console.log(options.currentProduct);
        if(options.currentProduct != ''){
            changeShape(options.currentProduct, true);  
        }else{
            changeShape(designer.currentShape.productId, true);
        }
    ...
    $('#badge_font').styleSelector({
        styles: options.fonts
    });
    $('#badge_font_size').styleSelector({
        styles: options.fontSizes
    });

    function changeShape(productId, initial) { //src, ml, mi, pid) {

        var newShape = null;
console.log('sample'+productId);
        for (i = 0; i < options.shapes.length; i++) {
        ...
        if (newShape) {
            ...
            //Badge Settings to show or hide - Start     
            if (newShape.allow_bgcolor == 1) {
                $("#bgcolor_panel").show();
            } else {
                $("#bgcolor_panel").hide();
            }

            if (newShape.allow_text_color == 1) {
                $("#badge_editor").show();
                $("#text_color_set").show();
            } else {
                $("#text_color_set").hide();
            ...
            if (newShape.allow_fonts == 1) {
                $("#badge_editor").show();
                $("#badge_fonts_option").show();
            } else {
                $("#badge_fonts_option").hide();
            }

            if (newShape.allow_fonts == 0 && newShape.allow_text_color == 0) {
                $("#badge_editor").hide();
            } else {
                $("#badge_editor").show();
            }
            //Added on July 31 2014
            if (newShape.allow_bgbrush == 1) {
                $("#badge_bg_brush").show();
            } else {
                $("#badge_bg_brush").hide();
            }

            //Badge Settings to show or hide - End
            ...
            var images = $(".badge_logo[id!='bd-logo']");
            ...
            for (i in newShape.opts) {
                ...
                po_text += '<tr><td><p>&nbsp;</p>';
                if (newShape.opts[i][3] == '1') {
                    po_text += '<span class="red_star">*</span>';
                }
                ...
                switch (newShape.opts[i][2]) {
                    case "0":
                        ...
                        if (newShape.opts[i][3] == '1') {
                            po_text += ' validate="required:true" ';
                        } //Mandatory
    ...
    $('.tab_page').bind("click", function(event) {
        if ($(this).attr('id') == 'tab_rgb') {
            $('#pms_colors').css('display', 'none');
            $('#rgb_colors').css('display', 'block');
        } else {
            $('#rgb_colors').css('display', 'none');
            $('#pms_colors').css('display', 'block');
        }
                    ...
                    if (designer.currentBorder != oldBorder) {
                        ...
                        if (oldBorder) {
                            if ((designer.currentBorder.width < oldBorder.width) || (designer.currentBorder.height < oldBorder.height)) {
                                ...
                                $('.badge_text').each(function() {
                                    relocateControl(this);
                                });
                                $('.badge_logo').each(function() {
                                    relocateControl(this);
                                });
                            }
                        }
    ...
    function selectBadgeText(ctrl) {
        ...
        if (ctrl) {
            ...
            $('#badge_text').attr('disabled', false);

            $('#badge_text').val($(ctrl).data("lines"));

            $('#badge_font').attr('disabled', false);
            $('#badge_font option').each(function() {
                ...
                ctrlfont = ctrlfont.replace('"', "");
                ctrlfont = ctrlfont.replace('"', "");
                ctrlfont = ctrlfont.replace("'", "");
                ctrlfont = ctrlfont.replace("'", "");
                curfont = curfont.replace('"', "");
                curfont = curfont.replace('"', "");
                curfont = curfont.replace("'", "");
                curfont = curfont.replace("'", "");
            ...
            $('#badge_font_size').attr('disabled', false);
            ...
            $('#badge_font_bold').attr('disabled', false);
            $('#badge_font_bold').attr('checked', false)
            if (($(ctrl).css('font-weight') == 'bold') || ($(ctrl).css('font-weight') == '700')) { // 700 - for IE8
                $('#badge_font_bold').attr('checked', 'checked')
            }

            $('#badge_font_italic').attr('disabled', false);
            $('#badge_font_italic').attr('checked', false)
            if ($(ctrl).css('font-style') == 'italic') {
                $('#badge_font_italic').attr('checked', 'checked')
            }

            $('#badge_font_underline').attr('disabled', false);
            $('#badge_font_underline').attr('checked', false)
            if ($(ctrl).css('text-decoration') == 'underline') {
                $('#badge_font_underline').attr('checked', 'checked')
            }

            $('#badge_delete_text').attr('disabled', false);
            $('#badge_delete_names').attr('disabled', false);

            $('.badge_text_color_rgb').attr('disabled', false);
    ...
    function bindDragEvents(className) {
        ...
        $('.' + className).bind("dragstart", function(event) {
            ...
            if ($(this).attr('dmode') == 'resize') {
                $($(this).children()[0]).css({
                    width: parseInt($(this).attr('iwidth')) + newW,
                    height: parseInt($(this).attr('iheight')) + newH
                });
            } else {
                //$('#debug').html('Designer left: ' + dsgnP.left + ', offsetX: ' + event.offsetX + ', new left: ' + newP.left + ', new top: ' + newP.top + ', text width: ' + $(this).width() + ', text height: '+ $(this).height());
    ...
    $('#badge_text_add_lines').bind("click", function(event) {
        ...
        if (((max_lines_count > -1) && (current_lines < max_lines_count)) || max_lines_count == -1) {
            ...
            if ((current_lines % 2) == 0) {
                numline = 2;
            } else {
                numline = 1;
            } //restrict added text lines
    ...
    $(document).on("click", '.update_text_confirm', function(event) {
    ...
    //edit text list
    $(document).on("click", '.text_list_edit', function(event) {
    ...
    $(document).on("click", '.text_list_delete', function(event) {
        ...
        if ($('.badge_container').length == 0) {
            $("#badge_text_add_lines").attr("disabled", false);
        }
