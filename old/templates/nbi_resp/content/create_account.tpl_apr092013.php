...
function getStateList(country){	   
	...
	function(data){
		...
		$('#available_states').html(data.returnValue);
	},	   		
	"json"
	);   
}
    ...
    if ($process == true) {
      if ($entry_state_has_zones == true) {
        ...
        echo tep_draw_input_field('state');
		  ...
		  //echo tep_draw_input_field('state');
		  $aus_zones_query = tep_db_query("select zone_name from " . TABLE_ZONES . " where zone_country_id = '13' order by zone_name");
			
			while ($aus_zones_values = tep_db_fetch_array($aus_zones_query)) {
		
			  $aus_zones_array[] = array('id' => $aus_zones_values['zone_name'], 'text' => $aus_zones_values['zone_name']);
	
			}
			echo tep_draw_pull_down_menu('state', $aus_zones_array);	
    }
...
?>
              <tr>
                <td class="main"><?php echo ENTRY_COUNTRY; ?></td>
				 <td class="main"><?php echo tep_get_country_list('country',13,"id='countries_list' style='width:200px;'") . '&nbsp;' . (tep_not_null(ENTRY_COUNTRY_TEXT) ? '<span class="inputRequirement">' . ENTRY_COUNTRY_TEXT . '</span>': ''); ?></td>
