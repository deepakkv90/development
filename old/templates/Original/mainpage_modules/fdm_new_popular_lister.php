  ...
  for ($x = 0; $x < max($no_listing1, $no_listing2); $x++) {
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        case 'FILE_LIST_NEW_DOWNLOADS':
          $lc_align = 'center';
          $lc_text = '<a href="' . tep_href_link(FILENAME_FILE_DETAIL, 'files_id=' . $listing1[$x]['files_id']) . '">' . tep_image(DIR_WS_IMAGES . 'file_icons/' . $listing1[$x]['icon_large'], $listing1[$x]['files_descriptive_name'], FDM_LARGE_ICON_IMAGE_WIDTH, FDM_LARGE_ICON_IMAGE_HEIGHT) . '</a><br>&nbsp;<a href="' . tep_href_link(FILENAME_FILE_DETAIL, 'files_id=' . $listing1[$x]['files_id']) . '">' . $listing1[$x]['files_descriptive_name'] . '</a>';
          break;
        case 'FILE_LIST_POPULAR_DOWNLOADS':
          $lc_align = 'center';
          $lc_text = '<a href="' . tep_href_link(FILENAME_FILE_DETAIL, 'files_id=' . $listing2[$x]['files_id']) . '">' . tep_image(DIR_WS_IMAGES . 'file_icons/' . $listing2[$x]['icon_large'], $listing2[$x]['files_descriptive_name'], FDM_LARGE_ICON_IMAGE_WIDTH, FDM_LARGE_ICON_IMAGE_HEIGHT) . '</a><br>&nbsp;<a href="' . tep_href_link(FILENAME_FILE_DETAIL, 'files_id=' . $listing2[$x]['files_id']) . '">' . $listing2[$x]['files_descriptive_name'] . '</a>';
          break;
      }
