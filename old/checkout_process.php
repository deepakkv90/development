...
if(is_array($cart->contents)) {   	      
	...
	foreach ($cart->contents as $cid=>$cont){
		...
		if($prod['badge_data']!="") {
			...
			if(!empty($prod['products_text']) && empty($prod['badge_namefile'])){
			...
			for ($i=0; $i<count($namefiles); $i++){
				if(!empty($namefiles[$i])) {
					...
					$nameFilesUploaded = "users_badges/".$filename_for_all ."_namelist_" . ($i+1) . ".".$namefile['extension'];
...
$email_order = "<div style='text-align:center; margin:0 auto; clear:both;'><img src='".HTTP_SERVER."/images/npi-nbi.jpg' alt='NPI to NBI'/></div><br/>";
