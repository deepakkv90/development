...
// Update the banner click statistics
  ...
  //Modified July 06 2011
// Display a banner with html text
  function tep_display_banner_with_text($action, $identifier) {
    if ($action == 'dynamic') {
      $banners_query = tep_db_query("select count(*) as count from " . TABLE_BANNERS . " where status = '1' and banners_group = '" . $identifier . "'");
      $banners = tep_db_fetch_array($banners_query);
      if ($banners['count'] > 0) {
	...
	$banner_content['id'] = $banner['banners_id'];
	$banner_content['title'] = $banner['banners_title'];
	$banner_content['image'] = $banner['banners_image'];
	$banner_content['text'] = $banner['banners_html_text'];
	$banner_content['link'] = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&amp;goto=' . $banner['banners_id']) . '" target="_blank">' . tep_image(DIR_WS_IMAGES . $banner['banners_image'], $banner['banners_title']) . '</a>';
