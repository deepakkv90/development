...
/*
 ...
 $Id: asszones.php,v 1.20 2003/06/15 19:48:09 thomasamoulton Exp $
...
// class constructor
   function asszones() {
     $this->code = 'asszones';
     $this->title = MODULE_SHIPPING_ASSZONES_TEXT_TITLE;
     $this->description = MODULE_SHIPPING_ASSZONES_TEXT_DESCRIPTION;
     if (defined('MODULE_SHIPPING_ASSZONES_SORT_ORDER')) {
       $this->sort_order = (int)MODULE_SHIPPING_ASSZONES_SORT_ORDER;
     } else {
       $this->sort_order = '';
     }

     $this->icon = '';
     if (defined('MODULE_SHIPPING_ASSZONES_TAX_CLASS')) {
       $this->tax_class = (int)MODULE_SHIPPING_ASSZONES_TAX_CLASS;
     } else {
       $this->tax_class = 0;
     }

     if (defined('MODULE_SHIPPING_ASSZONES_STATUS')) {
       $this->enabled = ((MODULE_SHIPPING_ASSZONES_STATUS == 'True') ? true : false);
     } else {
       $this->enabled = false;
     }
...
// class methods
   function quote($method = '') {
     ...
     for ($i=1; $i<=$this->num_zones; $i++) {
       $countries_table = constant('MODULE_SHIPPING_ASSZONES_COUNTRIES_' . $i);
...
/*
     ...
     if ($dest_zone == 0) {
       ...
       $shipping = -1;
       $zones_cost = constant('MODULE_SHIPPING_ASSZONES_COST_' . $dest_zone);
       ...
       for ($i=0; $i<$size; $i+=2) {
         if ($shipping_weight <= $zones_table[$i]) {
           ...
           $shipping_method = MODULE_SHIPPING_ASSZONES_TEXT_WAY . ' ' . $order->delivery['country']['title'] . ' ';
           if ($shipping_num_boxes > 1) {
             //$shipping_method .= $shipping_num_boxes . 'x ';
           }
           //$shipping_method .= $shipping_weight . ' ' . MODULE_SHIPPING_ASSZONES_TEXT_UNITS;
           break;
         }
       }
       ...
       if ($shipping == -1) {
         $shipping_cost = 0;
         $shipping_method = MODULE_SHIPPING_ASSZONES_UNDEFINED_RATE;
       } else {
         $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ASSZONES_HANDLING_' . $dest_zone);
       }
     }
      if ($shipping_cost == 0) {
        $shipping_cost = '0.00';
     ...
     $this->quotes = array('id' => $this->code,
                           'module' => MODULE_SHIPPING_ASSZONES_TEXT_TITLE,
     ...
     if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_ASSZONES_INVALID_ZONE;
   ...
   function check() {
     if (!isset($this->_check)) {
       $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ASSZONES_STATUS'");
   ...
   // elari - Added to select default country if not in listing
   function install() {
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Airmail Standard Service Method', 'MODULE_SHIPPING_ASSZONES_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_ASSZONES_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_ASSZONES_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
     for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
       if ($i == 1) {
      $default_countries = 'NZ';
         $shipping_table = '250:7.60,500:11.60,750:15.60,<br>1000:19.60,1250:23.60,1500:27.60,<br>1750:31.60,2000:35.60';
       }
       if ($i == 2) {
         $default_countries = 'CN,FJ,HK,IN,ID,IL,<br>JP,KR,MY,NC,PG,PH,<br>SG,SB,LK,TW,TH,VN';
         $shipping_table = '250:8.85,500:14.10,750:19.35,<br>1000:24.60,1250:29.85,1500:35.10,<br>1750:40.35,2000:45.60';
       }
	   if ($i == 3) {
         $default_countries = 'CA,US';
         $shipping_table = '250:10.05,500:16.50,750:22.95,<br>1000:29.40,1250:35.85,1500:42.30,<br>1750:48.75,2000:55.20';
       }
	   if ($i == 4) {
         $default_countries = 'AT,BE,DK,FI,FR,DE,<br>GR,IE,IT,MT,NL,NO,<br>PL,ZA,ES,SE,CH,GB';
         $shipping_table = '250:11.95,500:20.30,750:28.65,<br>1000:37.00,1250:45.35,1500:53.70,<br>1750:62.05,2000:70.40';
       }
      
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_ASSZONES_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_ASSZONES_COST_" . $i ."', '" . $shipping_table . "', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_ASSZONES_HANDLING_" . $i ."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
     }
   }
   // elari - Added to select default country if not in listing
   ...
   function keys() {
     $keys = array('MODULE_SHIPPING_ASSZONES_STATUS', 'MODULE_SHIPPING_ASSZONES_TAX_CLASS', 'MODULE_SHIPPING_ASSZONES_SORT_ORDER');

     for ($i=1; $i<=$this->num_zones; $i++) {
       $keys[] = 'MODULE_SHIPPING_ASSZONES_COUNTRIES_' . $i;
       $keys[] = 'MODULE_SHIPPING_ASSZONES_COST_' . $i;
       $keys[] = 'MODULE_SHIPPING_ASSZONES_HANDLING_' . $i;
     }
