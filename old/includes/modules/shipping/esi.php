...
/*
 ...
 $Id: esi.php,v 1.20 2003/06/15 19:48:09 thomasamoulton Exp $
...
// class constructor
   function esi() {
     $this->code = 'esi';
     $this->title = MODULE_SHIPPING_ESIZONES_TEXT_TITLE;
     $this->description = MODULE_SHIPPING_ESIZONES_TEXT_DESCRIPTION;
     if (defined('MODULE_SHIPPING_ESIZONES_SORT_ORDER')) {
       $this->sort_order = (int)MODULE_SHIPPING_ESIZONES_SORT_ORDER;
     } else {
       $this->sort_order = '';
     }

     $this->icon = '';
     if (defined('MODULE_SHIPPING_ESIZONES_TAX_CLASS')) {
       $this->tax_class = (int)MODULE_SHIPPING_ESIZONES_TAX_CLASS;
     } else {
       $this->tax_class = 0;
     }

     if (defined('MODULE_SHIPPING_ESIZONES_STATUS')) {
       $this->enabled = ((MODULE_SHIPPING_ESIZONES_STATUS == 'True') ? true : false);
     } else {
       $this->enabled = false;
     }
...
// class methods
   function quote($method = '') {
     ...
     for ($i=1; $i<=$this->num_zones; $i++) {
       $countries_table = constant('MODULE_SHIPPING_ESIZONES_COUNTRIES_' . $i);
...
/*
     ...
     if ($dest_zone == 0) {
       ...
       $shipping = -1;
       $zones_cost = constant('MODULE_SHIPPING_ESIZONES_COST_' . $dest_zone);
       ...
       for ($i=0; $i<$size; $i+=2) {
         if ($shipping_weight <= $zones_table[$i]) {
           ...
           $shipping_method = MODULE_SHIPPING_ESIZONES_TEXT_WAY . ' ' . $order->delivery['country']['title'] . ' ';
           if ($shipping_num_boxes > 1) {
             //$shipping_method .= $shipping_num_boxes . 'x ';
           }
           //$shipping_method .= $shipping_weight . ' ' . MODULE_SHIPPING_ESIZONES_TEXT_UNITS;
           break;
         }
       }
       ...
       if ($shipping == -1) {
         $shipping_cost = 0;
         $shipping_method = MODULE_SHIPPING_ESIZONES_UNDEFINED_RATE;
       } else {
         $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ESIZONES_HANDLING_' . $dest_zone);
       }
     }
      if ($shipping_cost == 0) {
        $shipping_cost = '0.00';
     ...
     $this->quotes = array('id' => $this->code,
                           'module' => MODULE_SHIPPING_ESIZONES_TEXT_TITLE,
     ...
     if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_ESIZONES_INVALID_ZONE;
   ...
   function check() {
     if (!isset($this->_check)) {
       $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ESIZONES_STATUS'");
   ...
   // elari - Added to select default country if not in listing
   function install() {
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Express Courier International Method', 'MODULE_SHIPPING_ESIZONES_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_ESIZONES_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_ESIZONES_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
     for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
       if ($i == 1) {
      $default_countries = 'NZ';
         $shipping_table = '500:46.50,1000:52.50,2000:61.00,<br>3000:70.50,4000:90.50,5000:90.50';
       }
       if ($i == 2) {
         $default_countries = 'CN,FJ,HK,IN,ID,IL,<br>JP,KR,MY,NC,PG,PH,<br>SG,SB,LK,TW,TH,VN';
         $shipping_table = '500:49.00,1000:58.00,2000:68.50,<br>3000:83.50,4000:108.50,5000:108.50';
       }
	   if ($i == 3) {
         $default_countries = 'CA,US';
         $shipping_table = '500:53.50,1000:64.50,2000:77.50,<br>3000:96.50,4000:128.50,5000:128.50';
       }
	   if ($i == 4) {
         $default_countries = 'AT,BE,DK,FI,FR,DE,<br>GR,IE,IT,MT,NL,NO,<br>PL,ZA,ES,SE,CH,GB';
         $shipping_table = '500:57.00,1000:68.00,2000:85.50,<br>3000:105.50,4000:148.50,5000:148.50';
       }
      
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_ESIZONES_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_ESIZONES_COST_" . $i ."', '" . $shipping_table . "', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_ESIZONES_HANDLING_" . $i ."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
     }
   }
   // elari - Added to select default country if not in listing
   ...
   function keys() {
     $keys = array('MODULE_SHIPPING_ESIZONES_STATUS', 'MODULE_SHIPPING_ESIZONES_TAX_CLASS', 'MODULE_SHIPPING_ESIZONES_SORT_ORDER');

     for ($i=1; $i<=$this->num_zones; $i++) {
       $keys[] = 'MODULE_SHIPPING_ESIZONES_COUNTRIES_' . $i;
       $keys[] = 'MODULE_SHIPPING_ESIZONES_COST_' . $i;
       $keys[] = 'MODULE_SHIPPING_ESIZONES_HANDLING_' . $i;
     }
