<?php
/*
  $Id: usps.php,v 3.0.2 2008/17/03 14:36:42 datazen Exp $
 ...
 */
  ...
  class usps {
    ...
    function usps() {
      ...
      $this->code = 'usps';
      if (defined('MODULE_SHIPPING_USPS_TEXT_TITLE')) {
        $this->title = MODULE_SHIPPING_USPS_TEXT_TITLE;
      } else {
        $this->title = '';
      }
      if (defined('MODULE_SHIPPING_USPS_TEXT_DESCRIPTION')) {
        $this->description = MODULE_SHIPPING_USPS_TEXT_DESCRIPTION;
      } else {
        $this->description = '';
      }           
      if (defined('MODULE_SHIPPING_USPS_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_USPS_SORT_ORDER;
      } else {
        $this->sort_order = 0;
      }
      $this->icon = DIR_WS_ICONS . 'shipping_usps.gif';
      if (defined('MODULE_SHIPPING_USPS_TAX_CLASS')) {
      ...
      $this->intl_types = array('GXG' => 'Global Express Guaranteed (GXG)',// Updated 4th Jan 09
                                ...
                                'EXI' => 'Express Mail International',// Updated 4th Jan 09
                                'EXI Ft Env' => 'Express Mail International Flat Rate Envelope', // Updated 4th Jan 09
                                'PMI' => 'Priority Mail International',
                                'PMI FR Env' => 'Priority Mail International Flat Rate Envelope',
                                'PMI FR BoxSM' => 'Priority Mail International Small Flat Rate Box', 
                                'PMI FR BoxMD' => 'Priority Mail International Medium Flat Rate Box', 
                                'PMI FR BoxLG' => 'Priority Mail International Large Flat Rate Box', 
                                '1st-Class Mail Int' => 'First-Class Mail International ' . $this->FirstClassIntType // Updated 4th Jan 09
                                );
    ...
    function install() {
      ...
      // usps V3 API password depricated
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Domestic Shipping Methods', 'MODULE_SHIPPING_USPS_TYPES', 'EXPRESS, PRIORITY, FIRST CLASS, PARCEL, BMP, LIBRARY, MEDIA,', 'Select the domestic services to be offered:', '6', '14', 'tep_cfg_select_multioption(array(\'EXPRESS\', \'PRIORITY\', \'FIRST CLASS\', \'PARCEL\',\'BPM\',\'MEDIA\'), ',  now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Int\'l Shipping Methods', 'MODULE_SHIPPING_USPS_TYPES_INTL', '', '<br>GXG=Global Express Guaranteed<br>PMI=Priority Mail International<br>EXI=Express Mail International<br>FR=Flat Rate<br>Env=Envelope BoxSM= Small Box BoxMD= Medium Box BoxLG= Large Box', '6', '15', 'tep_cfg_select_multioption(array(\'GXG\', \'GXG Non-Doc Rect\', \'GXG Non-Doc Non-Rect\', \'EXI\', \'EXI Ft Env\', \'PMI\', \'PMI FR Env\', \'PMI FR BoxSM\', \'PMI FR BoxMD\', \'PMI FR BoxLG\', \'1st-Class Mail Int\'), ', now() )");
    ...
    function _getQuote() {
      ...
      if ($http->Connect($usps_server, 80)) {
        ...
        $http->addHeader('User-Agent', 'CRELoaded');
        $http->addHeader('Connection', 'Close');
        if ($http->Get('/' . $api_dll . '?' . $request)) $body = $http->getBody();
        // mail('fullemailaddresshere','USPS rate quote response',$body,'From: <fullemailaddresshere>'); //Debug tool
      ...
      if ($order->delivery['country']['id'] == SHIPPING_ORIGIN_COUNTRY) {      
        ...
        for ($i=0; $i<$n; $i++) {
          if (strpos($response[$i], '<Rate>')) {        
            ...
            $service = $regs[1];
            if (ereg('Express', $service)) $service = 'Express Mail';
            if (ereg('Priority', $service)) $service = 'Priority Mail';            
            ...
            if ($transit) {        
              switch ($service) {
                ...
                case 'First-Class Mail': 
                  $time = 'Estimated 1 - 5 ' . MODULE_SHIPPING_USPS_TEXT_DAYS;
                  break;
