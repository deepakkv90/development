...
else
{
  ...
  $products_price11=$currencies->display_price(sprintf("%01.2f",$products_price11),tep_get_tax_rate($product_tax_id));
    ...
    //mod to remove duplicates when number of qty fields increased
    $qty_array = array_unique($qty_array);
    

$quantitytable = '<!-- QTY TABLE GENERATION BOF -->' . "\n";
//Loop how many qty avaible. -1 is fore removing blank 
//for($i = 0;$i <= PRODUCT_QTY_PRICE_LEVEL ; $i++){
for($i = 0;$i <= sizeof($qty_array)-1 ; $i++){
  if($i == 0){
      ...
      $quantitytable .= '  <tr>' . "\n"; 
      // bug #4254
      $quantitytable .= ' <td class="smalltext">' . $qty_array[$i]. ' - '; 
