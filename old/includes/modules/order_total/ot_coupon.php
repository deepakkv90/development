...
$order_total_tmp = $order_total - $exclude_product_price;
  ...
  if ($this->calculate_tax != 'None') { //Fred - changed from 'none' to 'None'!
	...
	//$this->coupon_description_code
	
	 $coupon_tax = tep_get_tax_rate($this->tax_class);	 
	 //$coupon_tax_amt = tep_calculate_tax($od_amount,$coupon_tax);	 
	 $coupon_tax_amt = tep_round(($od_amount - (($od_amount * 100)/(100 + $coupon_tax))),2);
	 	 
	 $order->info["discount_incl_tax"] = $od_amount; 	
	 $order->info["discount_excl_tax"] = $od_amount - $coupon_tax_amt; 
	 $order->info["discount_tax_amount"] = $coupon_tax_amt;
			
	 if ($od_amount > 0) {

		$order->info['total'] = $order->info['total'] - $od_amount;
		//$order->info['total'] = $order->info['total'] - tep_round($od_amount,2); //July 2012

		//$this->output[] = array('title' => $this->title . ':' . $this->coupon_code .':','text' => '<b>-' . $currencies->format($od_amount) . '</b>', 'value' => tep_round($od_amount,2)); //Fred added hyphen
		
		$this->output[] = array('title' => $this->title . ':' . $this->coupon_code .'(Excl. Tax):','text' => '<b>-' . $currencies->format($order->info["discount_excl_tax"]) . '</b>', 'value' => tep_round($order->info["discount_excl_tax"],2));
...
global $customer_id, $currencies, $language;
  ...
  //For check the contract discount if enable, we need to disable the redeem coupon - Feb 07 2011
  ...
  if ($coupon_result['coupon_type'] != 'G') {
    ...
    if (tep_db_num_rows($coupon_count_customer)>=$coupon_result['uses_per_user'] && $coupon_result['uses_per_user'] > 0) {
      ...
      tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_INVALID_USES_USER_COUPON . $coupon_result['uses_per_user'] . TIMES ), 'SSL'));
		...
		$tmp_order_total = $order->info['total']; //Feb 08 2011
		...
		$tmp_order_total = $tmp_order_total + ($order->info['shipping_cost'] * ($order->info['tax_class_id']/100)); //Feb 07 2011
		...
		$_SESSION['gv_redeem_amount'] = $coupon_amount; 
...
function calculate_credit($amount) {
  ...
  if (isset($cc_id) ) {
    ...
    /*** Coupon Descrption BOF ***/

	$coupon_description_query = tep_db_query("select coupon_description from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $cc_id . "'");

	if (tep_db_num_rows($coupon_description_query) !=0 ) {

		$coupon_description_result = tep_db_fetch_array($coupon_description_query);

		$this->coupon_description_code = $coupon_description_result['coupon_description'];
    ...
    if (tep_db_num_rows($coupon_query) !=0 ) {
	  ...
	  //for to add gst tax with shipping cost - Feb 04 2011
	  $gst_for_ship = $order->info['shipping_cost'] * ($order->info['tax_class_id']/100);

      if ($get_result['coupon_type']=='S') $c_deduct = $order->info['shipping_cost'] + $gst_for_ship;
      ...
      if ($get_result['coupon_type']=='S' && $get_result['coupon_amount'] > 0 ) $c_deduct = $order->info['shipping_cost'] + $gst_for_ship + $get_result['coupon_amount'];
