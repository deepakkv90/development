...
/*
function add_crm_account($data_arr)
{
	$url = "http://www.namebadgesinternational.com.au/CRM-success/OrderFromCre.php";	
	$fields = "post=true";
...
function manage_crm_order($order_id)
{
	...
	//mail("ananthan@indusnet.co.in", "TEST BEFORE T", "TEST BODY TEST".$order_id, "nbi.com");
	...
	$order_status = $order_stat_array['orders_status_name'];
  	$order_from = "Shopping Cart Frontend";
...
$admin_emails = tep_db_query("SELECT admin_firstname, admin_lastname, admin_email_address FROM admin");    

require_once(DIR_WS_TEMPLATES . 'Ajparkes1' . DIRECTORY_SEPARATOR . 'bd' . DIRECTORY_SEPARATOR . 'CreateZipFile.inc.php');
...
while ($email = tep_db_fetch_array($admin_emails)) {
 ...
 if(is_array($cart->contents)){
   ...
   foreach ($cart->contents as $cid=>$cont){
     ...
     $prod = tep_db_fetch_array(tep_db_query("SELECT products_image,  badge_data, products_text FROM products WHERE products_id = ".intval($cid)));

     if(!isset($files[0])){       

       $files[0] = array('file'=>file_get_contents(DIR_WS_IMAGES . $prod['products_image']), 'name'=>'example.png');

     }     

     //$create_zip_file->addFile($fileContents=file_get_contents(DIR_WS_IMAGES.$prod['products_image']), $prod['products_image']);

	

	if($productTextString == '')	{

		$productTextString = $prod['products_text'];

	}else	{

		$productTextString = $productTextString.",".$prod['products_text'];

	}

	

     $znames = split('{zname}',$cid);

     $nms = split('{zname}',$znames[0]);

     if(isset($znames[1])){

      $create_zip_file->addFile($fileContents=file_get_contents(DIR_WS_IMAGES.'temp'.DIRECTORY_SEPARATOR.$znames[1]), 'product_'.$nms[0].'.zip');

     }

     $content_bage .= '<b>'.'product_'.$nms[0].'.zip<br />';
	...
	//added Dec 10 2010
     ...
     require_once(dirname(__FILE__).'/templates/'.TEMPLATE_NAME.'/bd/badge_desc.php');

     $badge = new Badge($prod['badge_data']);

     $content_bage .= $badge->description().'<br />';

   }   

   $content_bage = str_replace('<br />', "<br />\n", $content_bage);

   $content_bage = str_replace('</tr>', "</tr>\n", $content_bage);

   $content_bage = str_replace('</th><td>', "  ", $content_bage);
	...
	$textFileName = $randamNumber.'.txt';

	$fh 		  = fopen(DIR_FS_CATALOG . 'images/users_badges/'. $textFileName, 'w') or die("can't open file");

	fwrite($fh, $productTextString);
   ...
   /// Adding Text File ///
	/// Adding Text File /// //hided Nov 12, 2010
   //$create_zip_file->addFile($fileContents=file_get_contents(DIR_FS_CATALOG . 'images/users_badges/'.$textFileName), 'product_text.txt'); 
   ...
   //$files[1] = array('file'=>$create_zip_file->getZippedfile(), 'name'=>'archive.zip');

 }

  $html = file_get_contents(DIR_WS_TEMPLATES . 'Ajparkes1' . DIRECTORY_SEPARATOR . 'bd' . DIRECTORY_SEPARATOR . 'admin_mail_teplate.html');
  ...
  $html = str_replace('%main_url%', HTTP_SERVER . DIR_WS_CATALOG, str_replace('%color_contents%', $content_bage, str_replace('%num_order%', $next_id , str_replace('%count_images%', count($order->products) , str_replace('%summ%', $order->info['total'].' '.$order->info['currency'], $html)))));

  //$html = str_replace('%color_contents%', $content_bage, str_replace('%num_order%', $next_id , str_replace('%count_images%', count($order->products) , str_replace('%summ%', $order->info['total'].' '.$order->info['currency'], $html))));

  

  //tep_mail_with_attach($email['admin_firstname'] . ' ' . $email['admin_lastname'], $email['admin_email_address'], 'New badge checkout', $html, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $files);  
  ...
  if($badgeAvail == 1){ 
    ...
    copy(DIR_WS_IMAGES . 'temp' . DIRECTORY_SEPARATOR . $zipName, DIR_WS_IMAGES . 'zip_bages' . DIRECTORY_SEPARATOR . 'order_' . $next_id . '.zip');    
	
  } 
   
  @unlink(DIR_WS_IMAGES . 'temp' . DIRECTORY_SEPARATOR . $zipName);
...
require_once(DIR_WS_TEMPLATES . 'nbi_au' . DIRECTORY_SEPARATOR . 'bd' . DIRECTORY_SEPARATOR . 'CreateZipFile.inc.php');
$files = array();
$zipName = '';
$badgeAvail = 0;
...
if(is_array($cart->contents)) {   	      
	...
	foreach ($cart->contents as $cid=>$cont){
		...
		if($prod['badge_data']!="") {
			...
			$badge_image_path = DIR_FS_CATALOG . 'images/'. $prod['products_image'];
...
//update customers table for orders count  - Sep 05 2011
  $selcust = tep_db_query("SELECT customers_orders_count FROM customers WHERE customers_id='".$_SESSION['customer_id']."'");
...
$cre_ord_array = array_merge($sql_data_array, array('order_id'=>$insert_id));
...
//add_crm_account($cre_ord_array);



//tep_redirect(tep_href_link(FILENAME_CHECKOUT_SUCCESS, 'order_id='. $insert_id, 'SSL'));

//echo(tep_href_link(FILENAME_CHECKOUT_SUCCESS, 'order_id='. $insert_id.'&customer_id='.$_SESSION['customer_id'], 'SSL'));
