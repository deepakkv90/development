...
if ($category_depth == 'nested') {
  ...
  $define_list = array('PRODUCT_LIST_MODEL' => PRODUCT_LIST_MODEL,
					   ...
					   'PRODUCT_LIST_MIN_ORDER_QTY' => PRODUCT_LIST_MIN_ORDER_QTY, // display min. order. qty. mod
  ...
  for ($i=0, $n=sizeof($column_list); $i<$n; $i++) {
    switch ($column_list[$i]) {
      ...
      case 'PRODUCT_LIST_MIN_ORDER_QTY':
          $select_column_list .= 'p.products_min_order_qty, ';
          break; 
...
//------------ Assigned TPL File for Product and Badge Application --------------------------/
$bbiProductsQuery = tep_db_query("SELECT p.products_id, p.products_model, p.products_image_med, pd.products_name, pc.categories_id FROM products AS p LEFT JOIN products_description AS pd ON pd.products_id = p.products_id LEFT JOIN products_to_categories AS pc ON pc.products_id = p.products_id WHERE p.products_status = 1 ORDER BY p.sort_order ASC");

	$other_categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . (int)253 . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by c.sort_order, cd.categories_name");
  ...
  if(!empty($CCID)){
		if(in_array($CCID,$OPID) && !empty($OPID)){
      
      //require(DIR_WS_TEMPLATES . TEMPLATE_NAME . '/' . TEMPLATENAME_BADGE_MAIN_PAGE);
